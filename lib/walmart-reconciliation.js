...
function variants(id) { return [id, ...(id.startsWith('0') ? [id.slice(1)] : []), ...(id.startsWith('00') ? [id.slice(2)] : [])]; }
function chooseMatch(remote, skuRows, upcRows, remoteIdentifierCount = 1) {
...
async function reconcileItem(pool, remote, context) {
  ...
  try {
    ...
    const { exact, fallback } = await candidates(db, remote);
    let decision = chooseMatch(remote, exact, fallback, context.identifierCount);
    ...
    decision = chooseMatch(remote, freshCandidates.exact, freshCandidates.fallback, context.identifierCount);
    if (decision.product && decision.product.id !== locked.id) throw fail('Catalog identity changed during reconciliation.');
    if (decision.status !== 'matched') { await db.query('rollback'); return decision; }
    if (decision.basis === 'upc' && context.sellerSkus.has(locked.sku)) { await db.query('rollback'); return { status: 'review', reason: 'Catalog SKU is reserved for an exact seller SKU match.' }; }
    ...
    return { status: 'linked', basis: decision.basis, catalogSku: locked.sku, sellerSku: remote.sku, productId: locked.id, publishedStatus: listing.publishedStatus, identifier: listing.matchedIdentifier };
...
async function runReconciliation({ client, job, write, read, check, persist, record, pool, invalidate }) {
  ...
  try {
    for (let i = 0; i < page; i++) {
      for (const remote of (await read(pageKey(job.id, i))).rows) {
        ...
        const row = { sellerSku: remote.sku, ...result }; record(row);
