...
function normalizeSourceOrderCompletion(order, options = {}) {
  ...
  const openReturn = [...(order.returns || []), ...(options.relatedReturns || [])].some((entry) => !["closed", "resolved", "done", "canceled", "cancelled", "completed"].includes(normalize(entry.status || entry.returnStatus)));
