...
// Recover the explicit datadump quantity without treating it as physical stock.
// Never infer supplier stock from aggregate qty or replace an existing source row.
function withDataWarehouseStock(item = {}) {
  const rows = Array.isArray(item.warehouseStock) ? item.warehouseStock : [];
  const stamp = item.productDumpUpdatedAt || item.stockUpdatedAt;
  const qty = item.stockQty;
  if (!isDataWarehouseImportedProduct(item) || !Number.isFinite(Date.parse(stamp)) || qty === '' || qty == null
    || !Number.isFinite(Number(qty)) || Number(qty) < 0 || rows.some(isDataWarehouseLocation)) return item;
  return upsertDataWarehouseStock({ ...item, warehouseStock: [...rows] }, Number(qty), stamp);
}
