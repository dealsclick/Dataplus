...
async function deleteByPrefix(prefix) {
  try {
    ...
    let deleted = 0;
    for await (const keys of activeClient.scanIterator({ MATCH: `${prefix}*`, COUNT: 250 })) {
      const entries = Array.isArray(keys) ? keys : [keys];
      if (entries.length) deleted += await activeClient.del(entries);
    }
    return deleted;
  } catch (error) {
    lastError = error?.message || "Unable to clear Redis cache";
    lastErrorAt = new Date().toISOString();
    return 0;
  }
}
...
module.exports = { getJson, setJson, deleteByPrefix, status };
