const defaults = {
  ...
  walmartPricingMode: "cost-plus", walmartPriceMarkupPercent: 30, walmartMinMarginPercent: 15,
  walmartMinimumPrice: 0, walmartRoundingRule: "none", walmartOrderScheduleHours: 1,
...
function applyWalmartSettings(current, body) {
  ...
  if (next.walmartPricingMode && !["cost-plus", "higher-of-product-or-cost", "product-price"].includes(next.walmartPricingMode)) invalid("walmartPricingMode");
  if (next.walmartRoundingRule && !["none", "nearest .99", "nearest .95", "round up"].includes(next.walmartRoundingRule)) invalid("walmartRoundingRule");
