...
function createOrderImporter({pool,init,transaction,access,audit}) {
  async function channel(client,scope,id) {
    ...
    const row=(await client.query('select id,name,source_key,enabled from workspace_manual_channels where tenant_id=$1 and company_id=$2 and id=$3 for share',[scope.tenantId,scope.companyId,id])).rows[0];
  ...
  return {
    async upload(user,scope,input){
      ...
      return transaction(async client=>{
        ...
        const row=(await client.query('insert into workspace_order_imports(tenant_id,company_id,source,filename,file_hash,payload,created_by,channel_id) values($1,$2,$3,$4,$5,$6,$7,$8) returning *',[scope.tenantId,scope.companyId,source,String(input.filename).slice(0,240),parsed.hash,JSON.stringify({parsed,channelName:selected.name}),user.id,selected.id])).rows[0];
