...
const laneSql = `case when coalesce(raw->>'workerTask','') = any($4::text[]) then 'orders' when coalesce(raw->>'workerTask','') = any($5::text[]) then 'walmart' when ${scheduledSql} then 'background' else 'manual' end`;
