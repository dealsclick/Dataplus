...
function createWalmartClient({ channel, credentials, log = () => {}, env = process.env, fetchImpl = fetch, pause = ms => new Promise(resolve => setTimeout(resolve, ms)) }) {
  ...
  async function request(path, { method = 'GET', body, gate, jobId, correlationId, retryReads = true, onResponse } = {}) {
    ...
    let expectedContext;
    const maxAttempts = method === 'GET' && retryReads ? 6 : 4;
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (retryReads && method === 'GET' && (response.status === 429 || response.status >= 500) && attempt < maxAttempts - 1) {
        const retryAfter = String(response.headers.get('retry-after') || '').trim();
        const retryAfterMs = /^\d+(\.\d+)?$/.test(retryAfter)
          ? Number(retryAfter) * 1000
          : Math.max(0, Date.parse(retryAfter) - Date.now());
        const exponentialMs = Math.min(120000, 1000 * 2 ** attempt);
        await pause(Math.min(120000, Math.max(exponentialMs, retryAfterMs || 0)));
        continue;
      }
