...
function createWalmartClient({ channel, log = () => {}, env = process.env, fetchImpl = fetch, pause = ms => new Promise(resolve => setTimeout(resolve, ms)) }) {
  ...
  async function context(operation) {
    ...
    const clientId = env[`${prefix}CLIENT_ID`], secret = env[`${prefix}CLIENT_SECRET`];
    if (!clientId || !secret) throw fail(`Configure ${prefix}CLIENT_ID and ${prefix}CLIENT_SECRET on the server.`, 409);
    return { current, origin: ORIGINS[environment], clientId, secret };
  }
  const headers = () => ({ Accept: 'application/json', 'WM_SVC.NAME': 'Walmart Marketplace', 'WM_MARKET': 'us', 'WM_QOS.CORRELATION_ID': crypto.randomUUID(), ...(env.WALMART_CHANNEL_TYPE ? { 'WM_CONSUMER.CHANNEL.TYPE': env.WALMART_CHANNEL_TYPE } : {}) });
  let tokenKey = '';
  async function access(ctx) {
    const key = crypto.createHash('sha256').update(ctx.origin + ctx.clientId + ctx.secret).digest('hex');
    if (key !== tokenKey) { token = ''; expires = 0; tokenKey = key; }
    if (token && expires > Date.now()) return token;
    if (!pending) pending = (async () => {
      const response = await fetchImpl(`${ctx.origin}/v3/token`, { method: 'POST', redirect: 'error', signal: AbortSignal.timeout(30000), headers: { ...headers(), Authorization: `Basic ${Buffer.from(`${ctx.clientId}:${ctx.secret}`).toString('base64')}`, 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'grant_type=client_credentials' });
  ...
  async function request(path, { method = 'GET', body, gate, jobId, correlationId } = {}) {
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      const contextId = `${ctx.current.id}:${ctx.origin}`;
      ...
      if (`${latest.current.id}:${latest.origin}` !== expectedContext) throw fail('Walmart environment changed during authorization.', 409);
      let response;
      try {
        response = await fetchImpl(ctx.origin + path, { method, redirect: 'error', signal: AbortSignal.timeout(45000), headers: { ...requestHeaders, ...(ctx.origin === ORIGINS.sandbox ? { WM_SANDBOX: 'v2' } : {}), 'WM_SEC.ACCESS_TOKEN': accessToken, ...(body !== undefined ? { 'Content-Type': 'application/json' } : {}) }, ...(body !== undefined ? { body: JSON.stringify(body) } : {}) });
      } catch {
      ...
      if (!response.ok || data.errors?.length || data.errors?.error?.length) {
        ...
        throw fail(`Walmart HTTP ${response.status}${details ? `: ${details}` : '. Review permissions, input, or API availability.'}`, 502);
      }
      return data;
    }
...
function identifier(product) {
  ...
  const value = gtin || upc;
  const kind = value.length === 14 ? 'gtin' : value.length === 12 ? 'upc' : value.length === 13 ? 'ean' : '';
  if (!kind || !/^\d+$/.test(value)) throw fail('A valid 12-digit UPC, 13-digit EAN, or 14-digit GTIN is required. Preserve leading zeros.');
...
function mergeOrderLines(incoming, existing = []) {
  return incoming.map(line => {
    ...
    return { ...old, ...line, ...(old ? { mappedSku: old.mappedSku, productId: old.productId, cost: old.cost, ...(old.skuMappedAt ? { sku: old.sku, originalSku: line.sku } : {}) } : {}) };
  });
}
