...
async function runBulkLaunch(d) {
  ...
  try {
    ...
    const sellerExists = async sku => {
      ...
      if (state.selectionTotal > 100) return false;
      let data;
    ...
    // Download the seller catalog once, indexed by exact seller SKU, without changing links.
    if (!state.sellerComplete) {
      const data = await request(`/v3/items?${new URLSearchParams({ nextCursor: state.sellerCursor, limit: '200' })}`);
      ...
      if (next && (next === state.sellerCursor || await read(`${prefix}.cursor.${hash(next)}`))) throw new Error('Walmart repeated a seller-catalog cursor.');
      for (const item of items) await write(`${prefix}.seller.${hash(item.sku)}`, { sku: item.sku });
      await write(`${prefix}.cursor.${hash(state.sellerCursor)}`, { seen: true });
      state.sellerCount = (state.sellerCount || 0) + items.length;
      const reported = Number(data.totalItems ?? data.totalCount);
      if (!next && Number.isFinite(reported) && state.sellerCount < reported) throw new Error('Walmart seller catalog ended before all listings were downloaded.');
      state.sellerCursor = next; state.sellerPage++; state.sellerComplete = !next;
    ...
    while (state.cursor < end && handled++ < CHUNK_SIZE) {
      ...
      if (!saved) {
        ...
        try {
          ...
          await finishRow(index, { sku: key, status: error.statusCode === 502 ? 'error' : 'blocked', error: error.message });
        }
      }
      state.cursor++;
    }
