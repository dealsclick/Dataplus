  ...
  async function run(job) {
    ...
    try {
      ...
      if (operation === 'existing-launch') {
        ...
        const selectedWarehouse = (await readDb()).warehouses?.find(row => row.id === configured.walmartWarehouseId);
        if (isDataWarehouseLocation(selectedWarehouse || {})) {
          const latest = await postgres.getPool().query("select status from operations_jobs where raw->>'workerTask'='product-dump-import' order by created_at desc limit 1");
          if (latest.rows[0]?.status !== 'success') throw fail('Supplier inventory sync was queued, but positive quantities are held until the newest datadump completes successfully.');
        }
        for (;;) {
          await check();
