...
function createWalmartMarketplace(deps) {
  ...
  async function checkReadiness(key, jobId, options = {}) {
    ...
    const row = { sku: p.sku, productId: p.id, title: p.title || '', identifier: id, status: lookupError ? 'error' : matched ? 'matched' : search.items.length ? 'full_setup' : 'not_found', ...(lookupError ? { error: lookupError.message } : {}), ...reference, existingOffer, newItem, checkedAt: stamp(), expiresAt: Date.now() + 86400000, fingerprint, ...lookupMeta };
