...
function createWalmartMarketplace(deps) {
  ...
  async function queue(operation, payload) {
    return lock('queue', async () => {
      ...
      if (existing && operation === 'match' && (existing.workerPayload?.actor !== payload.actor || digest(existing.workerPayload?.skus) !== digest(payload.skus))) throw fail('Another Walmart UPC match is queued or running. Wait for it to finish, then match this selection.', 409);
      ...
      if (operation === 'orders') {
        ...
        for (const sku of payload.skus) {
          ...
          try {
            ...
            const result = await client.request(`/v3/items/walmart/search?${new URLSearchParams({ [id.kind]: id.value, responseFormat: 'SPEC' })}`, { jobId: job.id });
            ...
            row = { sku: p.sku, title: p.title || '', identifier: id, status: match?.feedType === 'MP_ITEM_MATCH' ? 'matched' : match ? 'full_setup' : 'not_found', feedType: match?.feedType || '', productType: match?.productType || '', version: match?.version || '', packReviewRequired: Boolean(deps.packSize && deps.packSize(p) > 1) };
          } catch (error) { row = { sku, status: 'error', error: error.message }; }
          await check();
          rows.push(row); record(row);
          await write(`walmart.match.${job.id}`, { actor: payload.actor, environment: payload.environment, credentialKey: payload.credentialKey, rows, jobId: job.id, total: payload.skus.length, complete: false, updatedAt: stamp() });
          await persistJob(job, { processedRows: processed, totalRows: payload.skus.length, progressPercent: Math.round(processed / payload.skus.length * 100), message: `${processed}/${payload.skus.length} Walmart UPC lookups completed.` });
        }
        await write(`walmart.match.${job.id}`, { actor: payload.actor, environment: payload.environment, credentialKey: payload.credentialKey, rows, jobId: job.id, total: payload.skus.length, complete: true, updatedAt: stamp() });
  ...
  async function handle(req, res, url, actor, sendJson, parseBody) {
    ...
    try {
      ...
      if (req.method === 'GET' && action === 'status') {
        ...
        if (!skus.length || skus.length > 100) throw fail('Select between 1 and 100 catalog SKUs per UPC match.');
        await enabled();
        result = await queue('match', { skus, actor });
        ...
        result = { ...(batch || { rows: [], total: job.workerPayload.skus.length, complete: false }), jobId: job.id, jobNumber: job.jobNumber, status: job.status, message: job.message };
