...
function createWalmartMarketplace(deps) {
  ...
  async function submitLaunch(token, job, check) {
    ...
    return lock(`launch-product:${payload.environment}:${initial.productId}`, async () => {
  ...
  async function run(job) {
    ...
    try {
      ...
      } else if (operation === 'match' || operation === 'existing-launch') {
        ...
        for (const sku of (await read(`walmart.match.selection.${job.id}.${page}`)).keys) {
          ...
          try {
            if (operation === 'existing-launch') {
              const p = await product(sku), prior = await read(`walmart.launch-intent.${payload.environment}.${digest(p.id)}`);
              if (prior || await read(`walmart.listing.${payload.environment}.${digest(p.id)}`) || (payload.environment === 'production' && (p.walmartListing?.sku || p.walmartListing?.feedId))) {
                row = { sku: p.sku, status: prior?.state === 'submitting' ? 'needs_reconciliation' : 'skipped', message: 'Already linked or submitted. Verify the existing listing/feed.' };
              } else {
                let remote;
                try { remote = await client.request(`/v3/items/${encodeURIComponent(p.sku)}?productIdType=SKU`, { jobId: job.id }); }
                catch (error) { if (error.upstreamStatus !== 404) throw error; }
                if (remote) {
                  const items = remote.ItemResponse || remote.itemResponse;
                  if (!Array.isArray(items)) throw fail('Invalid seller item response; cannot safely launch.', 502);
                  if (items.length) throw Object.assign(fail('Seller SKU already exists on Walmart. Link or verify it instead.'), { resultStatus: 'skipped' });
                }
                const preview = await prepare(sku, {}, payload.actor, false, { existingOnly: true });
                if (preview.errors?.length) throw fail(preview.errors.map(e => `${e.field}: ${e.message}`).join('; '));
                row = await submitLaunch(preview.token, job, check);
              }
            } else row = payload.readiness ? await checkReadiness(sku, job.id) : await lookupProduct(sku, job.id);
        ...
        for (;;) {
          ...
          for (const row of rows) {
            ...
            if (payload.environment === 'production' && deps.saveListing && row.sku) {
              ...
              if (p?.sku === row.sku && p.walmartListing?.feedId === payload.feedId && row.ingestionStatus === 'SUCCESS') {
  ...
  async function handle(req, res, url, actor, sendJson, parseBody) {
    ...
    try {
      ...
      if (req.method === 'GET' && action === 'status') {
        ...
        await enabled();
        const job = await postgres.readOperationJob(url.searchParams.get('jobId'));
        if (!job || job.workerTask !== (action === 'launch/existing' ? 'walmart-existing-launch' : 'walmart-match') || job.workerPayload?.actor !== actor) throw fail('Your Walmart match job was not found.', 404);
        const s = await settings();
