...
function createWalmartMarketplace(deps) {
  ...
  async function queue(operation, payload) {
    return lock('queue', async () => {
      ...
      if (operation === 'existing-launch') await persistJob(job, { queuePriority: 30 });
  ...
  async function prepareBulkOffer(key, sellerExists, request) {
    ...
    launchGate(p, await readDb(), c.settings);
    ...
    return lock('launch-write', async () => {
      ...
      if (batch?.state !== 'submitted') {
        ...
        for (const row of rows) {
          ...
          try {
            const mapping = preview?.category ? await read(mappingKey(preview.category)) : null;
            if (!preview || preview.feedType !== 'MP_ITEM_MATCH' || preview.errors?.length || preview.fingerprint !== digest([p, mapping, c.settings, await credentialKey()])) throw fail('Product or channel settings changed during preparation. Run a fresh launch for this SKU.');
            launchGate(p, await readDb(), c.settings);
            accepted.push({ ...row, sku: p.sku, item: preview.payload.MPItem[0] });
      ...
      for (const row of batch.accepted) {
        await check();
        const submission = { sku: row.sku, productId: row.productId, feedId: batch.feedId, feedType: 'MP_ITEM_MATCH', status: 'submitted', jobId: job.id, batchKey, correlationId: batch.correlationId, at: batch.submittedAt };
