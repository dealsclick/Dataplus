...
function createWalmartMarketplace(deps) {
  ...
  const client = createWalmartClient({ channel, log, ...(deps.fetchImpl ? { fetchImpl: deps.fetchImpl } : {}) });
  ...
  async function prepare(key, overrides = {}, actor) {
    ...
    const preview = { token: crypto.randomUUID(), actor, productId: p.id, sku: p.sku, category, feedType, productType, version, price, identifier: id, identifierPackConfirmed: overrides.confirmIdentifierPack === true, payload, errors, createdAt: stamp(), expiresAt: Date.now() + 30 * 60000, fingerprint: digest([p, mapping, s]), channelId: c.id, environment: s.walmartEnvironment || 'production', state: 'preview' };
  ...
  async function operationPlan(kind, key, shipmentId) {
    ...
    return { kind, key, shipmentId, gate, path, body, method: ['inventory','price'].includes(kind) ? 'PUT' : 'POST', fingerprint: digest(source), environment: s.walmartEnvironment || 'production', channelId: c.id };
  }
  ...
  async function queue(operation, payload) {
    return lock('queue', async () => {
      ...
      const c = await channel();
      const job = await createJob({ section: operation === 'orders' ? 'Operations' : 'Products', category: operation === 'orders' ? 'Orders' : 'Channels', operation: `Walmart ${operation}`, direction: operation === 'orders' ? 'import' : 'sync', status: 'queued', phase: 'queued', workerTask, workerPayload: { ...payload, channelId: c.id, environment: c.settings.walmartEnvironment || 'production' }, fileName: `walmart-${operation}.ndjson`, message: `Walmart ${operation} queued.` });
    ...
    try {
      ...
      if (operation === 'orders') {
        ...
        try { await lock(`preview:${token}`, async () => {
          ...
          if (preview.fingerprint !== digest([p, mapping, c.settings])) throw fail('Product, mapping, or channel rules changed. Create a new preview.');
  ...
  async function handle(req, res, url, actor, sendJson, parseBody) {
    ...
    try {
      ...
      if (req.method === 'GET' && action === 'status') {
        ...
        result = { configured: Boolean(process.env[`${prefix}CLIENT_ID`] && process.env[`${prefix}CLIENT_SECRET`]), environment: s.walmartEnvironment || 'production', schedule: await read('walmart.orderSchedule'), taxonomy: { version: taxonomy?.version, count: taxonomy?.rows?.length || 0, updatedAt: taxonomy?.updatedAt } };
      } else if (req.method === 'POST' && action === 'connection/verify') {
        await client.request('/v3/orders?limit=1&shipNodeType=SellerFulfilled'); result = { message: 'Walmart authentication and order-read access verified. Item API permissions are checked when used.' };
