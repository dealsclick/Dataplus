...
function createWalmartMarketplace(deps) {
  ...
  async function checkReadiness(key, jobId) {
    await enabled();
    const p = await product(key), fingerprint = await readinessFingerprint(p);
    ...
    try {
      ...
      if (!search || typeof search !== 'object' || search.errors || !Array.isArray(search.items) || search.items.length > 1 || search.items.some(item => !['MP_ITEM_MATCH', 'MP_ITEM'].includes(item.feedType))) throw fail('Identifier lookup was invalid or ambiguous. Run the check again.');
    ...
    const row = { sku: p.sku, productId: p.id, title: p.title || '', identifier: id, status: lookupError ? 'error' : matched ? 'matched' : 'not_found', ...(lookupError ? { error: lookupError.message } : {}), existingOffer, newItem, checkedAt: stamp(), expiresAt: Date.now() + 86400000, fingerprint };
  ...
  async function queue(operation, payload) {
    return lock('queue', async () => {
      await enabled(operation === 'orders' ? 'walmartOrdersEnabled' : operation === 'launch' ? 'walmartLaunchEnabled' : operation === 'reconcile' ? 'walmartLinkExistingEnabled' : undefined);
      const workerTask = `walmart-${operation}`;
      const existing = await findActive(workerTask);
      if (existing && ['match', 'reconcile'].includes(operation) && (existing.workerPayload?.actor !== payload.actor || digest([existing.workerPayload?.readiness, existing.workerPayload?.skus, existing.workerPayload?.allFiltered, existing.workerPayload?.query, existing.workerPayload?.filters]) !== digest([payload.readiness, payload.skus, payload.allFiltered, payload.query, payload.filters]))) throw fail('Another Walmart UPC match is queued or running. Wait for it to finish, then match this selection.', 409);
  ...
  async function run(job) {
    ...
    const check = async () => {
      ...
      const c = await enabled(operation === 'orders' ? 'walmartOrdersEnabled' : operation === 'launch' ? 'walmartLaunchEnabled' : operation === 'reconcile' ? 'walmartLinkExistingEnabled' : undefined);
    ...
    try {
      ...
      if (operation === 'reconcile') {
        ...
        for (const sku of (await read(`walmart.match.selection.${job.id}.${page}`)).keys) {
          ...
          await persistJob(job, { processedRows: processed, totalRows: total, progressPercent: Math.round(processed / Math.max(total, 1) * 100), message: `${processed}/${total} Walmart ${payload.readiness ? 'readiness checks' : 'UPC lookups'} completed.` });
  ...
  async function handle(req, res, url, actor, sendJson, parseBody) {
    ...
    try {
      ...
      if (req.method === 'GET' && action === 'status') {
        ...
        await enabled();
        result = await queue('match', { skus, actor, readiness: body.readiness === true, ...(body.allFiltered === true ? { allFiltered: true, query: String(body.query || '').slice(0, 500), filters: object(body.filters) } : {}) });
