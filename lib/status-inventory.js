...
function requireChannel(channel, key) {
  ...
  if (key === 'walmart' && (settings.channelEnabled !== true || settings.walmartInventoryEnabled !== true)) throw new Error('Walmart inventory updates are disabled.');
  if (!['shopify', 'ebay', 'walmart'].includes(key)) requireInventoryChannel(channel);
}
...
function createStatusInventoryWorker({ postgres, persistJob, artifactsDir, readDb, log, ebayRequest, shopify, temuRequest, walmartZero, send = sendZeroRequest }) {
  return async function run(job) {
    ...
    for (;;) {
      ...
      for (const row of rows) {
        ...
        for (const key of ['shopify', 'ebay', 'temu', 'whatnot', 'tiktok', 'walmart']) {
          ...
          try {
            ...
            if (key === 'shopify') {
              ...
              if (key === 'walmart') {
                if (!walmartZero) throw new Error('Walmart inventory adapter is unavailable.');
                await walmartZero(latest.item.id, job.id);
              } else if (key === 'ebay') {
                const listing = latest.item.ebayListing || {};
                const targets = require('./ebay-variation-plan').listingTargets(latest.item);
                const failures = [];
                for (let offset = 0; offset < targets.length; offset += 25) {
                  const batch = targets.slice(offset, offset + 25);
                  const result = await ebayRequest(latest.db, '/sell/inventory/v1/bulk_update_price_quantity', { method: 'POST', body: { requests: batch.map(target => ({
                    sku: target.sku, shipToLocationAvailability: { quantity: 0 },
                    ...(target.offerId ? { offers: [{ offerId: target.offerId, availableQuantity: 0 }] } : {})
                  })) }, jobId: job.id });
                  for (const target of batch) {
                    const ack = result.responses?.find(r => r.sku === target.sku);
                    if (!ack || Number(ack.statusCode) < 200 || Number(ack.statusCode) >= 300 || ack.errors?.length) failures.push(`${target.sku}: ${ack?.errors?.map(e => e.message).join('; ') || 'eBay did not acknowledge zero inventory; check listing linkage.'}`);
                  }
                }
          ...
          log({ channel: { shopify: 'Shopify', ebay: 'eBay', temu: 'Temu', whatnot: 'Whatnot', tiktok: 'TikTok Shop', walmart: 'Walmart' }[key], transport: 'Job', method: 'APPLY', path: 'status-inventory', operation: 'Automatic status inventory protection', jobId: job.id, ok: !error, statusCode: error ? 422 : 200, message: `${item.sku}: ${error || 'Zero inventory accepted by channel.'}` });
        }
