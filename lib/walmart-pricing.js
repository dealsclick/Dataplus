...
async function runPricing({ job, client, read, write, lock, check, persist, now = Date.now }) {
  ...
  let response;
  try {
    response = await client.request('/v3/price/getPricingInsights', { method: 'POST', body: { pageNumber: state.page, sort: { sortField: 'GMVL30D', sortOrder: 'DESC' } }, jobId: job.id, retryReads: false, onResponse: async r => {
  ...
  if (rows.length && await read(`${runKey}.seen.${pageHash}`)) {
    state.pageRepeatAttempts = Number(state.pageRepeatAttempts || 0) + 1;
    await write(runKey, state);
    if (state.pageRepeatAttempts <= 2) {
      return defer(now() + 60000, `Walmart repeated pricing page ${state.page}; retry ${state.pageRepeatAttempts} of 2 scheduled.`);
    }
    await write(`${account}.sync`, { checkedAt: new Date(now()).toISOString(), jobId: job.id, count: state.processed, incomplete: true });
    return persist({
      status: 'warning', phase: 'complete', finishedAt: new Date(now()).toISOString(),
      processedRows: state.processed, totalRows: count, progressPercent: count ? Math.min(99, Math.round(state.processed / count * 100)) : 100,
      message: `${state.processed} of ${count} Walmart pricing insights cached. Walmart repeated page ${state.page} after two retries; existing cached prices were preserved.`
    });
  }
  ...
  state.processed += rows.length; state.page++; state.pageRepeatAttempts = 0; state.complete = state.processed >= count;
