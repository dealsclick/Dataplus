...
function retiredSupplier(item = {}, vendors = []) {
  ...
  return vendors.find(v => supplierUnavailable(v) && supplierTokens(v).some(t => tokens.includes(t)))
    || (markerApplies ? { retirement: marker } : null);
}

function supplierUnavailable(vendor = {}) {
  return !!vendor.retirement?.retiredAt || vendor.active === false || vendor.active === 0
    || ['false', '0'].includes(String(vendor.active).toLowerCase())
    || ['inactive', 'disabled'].includes(String(vendor.status || '').trim().toLowerCase());
}
...
function retirementLaunchReason(item = {}, vendors = []) {
  const supplier = retiredSupplier(item, vendors);
  return supplier && retirementPhysicalQty(item) <= 0
    ? `Supplier ${supplier.retirement?.retiredAt ? 'retired' : 'inactive'}: no verified physical stock. Review and approve a replacement source before launch.` : '';
}
...
module.exports={supplierTokens,supplierUnavailable,retiredSupplier,retirementPhysicalQty,retirementLaunchReason,createRetirementService};
