...
function parseReport(buffer) {
  ...
  return table.filter(row => row.some(v => String(v).trim())).map(row => {
    ...
    const url = get('Item Page URL').replace(/^http:\/\//i, 'https://'), id = get('Item ID') || url.match(/\/(\d+)(?:[?#].*)?$/)?.[1] || '';
    const price = get('Price'), lag = get('Fulfillment Lag Time');
    const productId = get('Product ID'), productIdType = get('Product ID Type').toUpperCase();
    return { sku: get('SKU'), upc: get('UPC') || (productIdType === 'UPC' || productIdType === 'EAN' ? productId : ''), gtin: get('GTIN') || (productIdType === 'GTIN' ? productId : ''), wpid: get('WPID') || (productIdType === 'WPID' ? productId : ''), itemId: id,
...
async function itemReport({client, job, read, write, check, persist, pause = ms => new Promise(r => setTimeout(r, ms)), fetchImpl = fetch, now = Date.now}) {
  ...
  const waitUntil = async until => {
    while (until > now()) { await check(); await pause(Math.min(30000, until - now())); }
  };
  const reportRead = async path => {
    for (let retry = 0; retry < 8; retry++) {
      await waitUntil(state.nextPollAt || 0); await check();
      let retryAt = now() + 900000;
      state.nextPollAt = now() + 300000; await write(doc, state);
      try { return await client.request(path, {jobId: job.id, retryReads: false, onResponse: async response => {
        if (response.status !== 429) return;
        const value = response.headers.get('retry-after');
        const delay = value && Number.isFinite(Number(value)) ? Number(value) * 1000 : Date.parse(value || '') - now();
        retryAt = now() + Math.max(900000, delay || 0);
      }}); } catch (error) {
        if (error.upstreamStatus !== 429) throw error;
        state.nextPollAt = retryAt; await write(doc, state);
        await persist({phase:'item-report-wait', message:'Walmart report request throttled; automatically retrying after ' + new Date(retryAt).toISOString()});
      }
    }
    throw fail('Walmart report allowance remains unavailable. Retry this job to reuse its request.');
  };
  for (let attempt = 0; attempt < 60; attempt++) {
    await check();
    const result = await reportRead(`/v3/reports/reportRequests/${encodeURIComponent(state.requestId)}`);
    ...
    if (status === 'READY') {
      state.nextPollAt = 0; // Download URL has a separate allowance from report status.
      const download = await reportRead(`/v3/reports/downloadReport?requestId=${encodeURIComponent(state.requestId)}`);
    ...
    await waitUntil(state.nextPollAt);
  }
