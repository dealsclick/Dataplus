...
function normalizeShippingRules(settings = {}) {
  ...
  // Custom limits may be stricter, but must never label an over-limit package Ground eligible.
  for (const key of ["shippingParcelMaxLength", "shippingParcelMaxGirth", "shippingParcelMaxWeight"]) {
    rules[key] = Math.min(rules[key], DEFAULT_SHIPPING_RULES[key]);
  }
  return rules;
}
...
function classifyShipping(item = {}, settings = {}) {
  ...
  const result = (shippingClass, shippingClassReason) => ({ shippingClass, shippingClassReason, dimensionalWeight,
    shippingMethod: ({ parcel: "FedEx Ground eligible", ltl: "LTL", missing_measurements: "Shipping review" })[shippingClass] });
  const override = String(read("shippingClassOverride") || "");
  if (override === "ltl") {
    return result("ltl", `Manual override: ${read("shippingOverrideReason") || "Operator classification"}`);
  }
  ...
  // Legacy parcel/oversize overrides are re-evaluated; neither can bypass Ground limits.
  return result("parcel", "Within configured FedEx Ground size and weight limits. Large-parcel surcharges and dimensional weight do not block selling.");
}
