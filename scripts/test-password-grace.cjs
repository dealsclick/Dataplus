...
const gate = source.slice(source.indexOf('  const ownPasswordMatch ='), source.indexOf('  if (req.method === "GET" && url.pathname === "/api/users")'));
...
(async () => {
  ...
  assert.equal(admin.companyChecks, gate.includes("companyOperationsHandler") ? 2 : 0);
