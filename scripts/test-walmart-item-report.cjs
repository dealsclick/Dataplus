...
let requested=0; const docs=new Map(); const args={job:{id:'t'},read:async k=>docs.get(k),write:async(k,v)=>docs.set(k,v),check:async()=>{},persist:async()=>{},pause:async()=>{},client:{request:async(path,opts)=>{if(opts?.method==='POST'){requested++;return {requestId:'r'};} if(path.includes('downloadReport'))return {downloadURL:'https://reports.walmart.com/file'};return {requestStatus:'READY'};}},fetchImpl:async()=>new Response(csv)};
