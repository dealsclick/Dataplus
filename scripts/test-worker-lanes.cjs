...
const {LANES, ORDER_TASKS, validateLane, summarizeWorkers, jobWorkerStatus, laneSql} = require('../lib/worker-lanes');
...
assert.equal(workers.workers.length,3);
...
const cases=[['manual launch',{workerTask:'walmart-bulk-launch'},'manual'],['Scheduled feed',{workerTask:'vendor-feed-import'},'background'],['manual order import',{workerTask:'walmart-orders'},'orders'],['scheduled returns',{workerTask:'ebay-return-import',scheduled:true},'orders'],['sync',{workerTask:'shopify-inventory-update',workerPayload:{scheduled:true}},'background'],['manual sync',{workerTask:'shopify-inventory-update',scheduled:false},'manual']];
for(const [name,raw,lane]of cases){await c.query('truncate lane_jobs');await c.query('insert into lane_jobs values($1,$2)',[name,raw]);const result=await c.query(`select ${laneSql.replaceAll('$4','$1')} lane from lane_jobs`,[ORDER_TASKS]);assert.equal(result.rows[0].lane,lane);}
await c.query('rollback');console.log('PASS SQL worker routing for manual, scheduled, orders and returns');
