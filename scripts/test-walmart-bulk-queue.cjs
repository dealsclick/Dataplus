...
(async()=>{
  ...
  try{
    ...
    const context={...require('../lib/worker-lanes'),getPool:()=>client,initRelationalSchema:async()=>{},nullableString:s=>s,crypto:require('node:crypto')};vm.createContext(context);vm.runInContext(source.slice(start,end),context);
    ...
    await client.query('truncate operations_jobs');
    for (const [id,task,scheduled] of [['manual','walmart-bulk-launch',false],['orders','walmart-orders',true],['background','vendor-feed-import',true]]) {
