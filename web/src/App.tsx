...
function OrderDetailWorkspace() {
  ...
  return <div className="grid gap-5">
    <PageHeader eyebrow="Operations / Order" title={String(order.orderNumber || orderId)} description={`${String(order.source || "Order")} / ${String(order.channelSource || "Unclassified sales channel")}`} action={<div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" asChild><a href="/orders">Back to orders</a></Button>{String(order.source || "").toLowerCase() === "shopify" && <Button size="sm" variant="outline" onClick={() => window.setTimeout(() => document.querySelector<HTMLElement>("[role='tab'][aria-controls*='fulfillment']")?.click(), 0)}><Truck className="size-4" /> Shipping & labels</Button>}<OrderActionsMenu order={order} busy={saving} onAction={runOrderAction} onRefresh={() => void load()} /></div>} />
