...
function OrderDetailWorkspace() {
  ...
  return <div className="grid gap-5">
    ...
    <Tabs defaultValue="items"><TabsList className="flex flex-wrap"><TabsTrigger value="items">Items & P&amp;L</TabsTrigger><TabsTrigger value="fulfillment">Fulfillment ({shipments.length})</TabsTrigger><TabsTrigger value="finance">Finance</TabsTrigger><TabsTrigger value="customer">Customer</TabsTrigger><TabsTrigger value="returns">Returns ({relatedReturns.length})</TabsTrigger><TabsTrigger value="documents">Documents ({documents.length})</TabsTrigger><TabsTrigger value="channel">Channel</TabsTrigger><TabsTrigger value="activity">Activity ({events.length})</TabsTrigger></TabsList>
      ...
      <TabsContent value="customer" className="grid gap-4 pt-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Detail label="Customer orders" value={numberLabel(Number(customerSummary.orderCount || 0))} /><Detail label="Lifetime value" value={moneyLabel(Number(customerSummary.lifetimeValue || 0))} /><Detail label="First order" value={dateLabel(String(customerSummary.firstOrderAt || ""))} /><Detail label="Most recent" value={dateLabel(String(customerSummary.lastOrderAt || ""))} /></div><Card><CardHeader><CardTitle className="text-sm">Customer contact</CardTitle></CardHeader><CardContent className="grid gap-2 text-sm"><p className="font-medium">{String(order.buyer || "Customer")}</p><p>{String(order.buyerEmail || "No email")}</p><p>{String(order.phone || "No phone")}</p><p className="text-muted-foreground">Customer identity is matched by Shopify customer ID when available, otherwise by email.</p></CardContent></Card></TabsContent>
