...
function ShippingLabelReadinessPanel({ orderId, order, onUpdated, shipmentDraft }: { orderId: string; order: Record<string, unknown>; onUpdated: () => Promise<void>; shipmentDraft?: Record<string, unknown> }) {
  ...
  return <Card><CardHeader><CardTitle className="text-sm">Shopify Shipping label readiness</CardTitle><CardDescription>Checks technical label prerequisites only. Label purchase stays separate from delivery-option selection.</CardDescription></CardHeader><CardContent className="grid gap-3"><Button size="sm" variant="outline" className="w-fit" onClick={() => void check()} disabled={loading}>{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />} Check readiness</Button>{result && <div className="rounded-md border p-3 text-sm"><Badge variant={result.ready ? "default" : "outline"}>{result.ready ? "Label prerequisites ready" : "Needs setup"}</Badge><p className="mt-2 text-muted-foreground">Shopify API: {result.apiVersion || "Not configured"}</p>{!result.ready && <ul className="mt-2 list-disc space-y-1 pl-5 text-muted-foreground">{(result.blockers || []).map((blocker) => <li key={blocker}>{blocker}</li>)}</ul>}</div>}{purchases.map((purchase) => { const labels = Array.isArray(purchase.labels) ? purchase.labels as Array<Record<string, unknown>> : []; const errors = Array.isArray(purchase.errors) ? purchase.errors as Array<Record<string, unknown>> : []; return <div key={String(purchase.id)} className="rounded-md border p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-medium">Shopify label purchase</p><p className="text-xs text-muted-foreground">{dateLabel(String(purchase.requestedAt || purchase.checkedAt || ""))}</p></div><div className="flex items-center gap-2"><Badge variant={String(purchase.status) === "PURCHASED" ? "default" : String(purchase.status) === "PURCHASE_FAILED" ? "destructive" : "secondary"}>{String(purchase.status || "PENDING_PURCHASE")}</Badge>{!Boolean(purchase.done) && <Button size="sm" variant="outline" disabled={loading} onClick={() => void refreshPurchase(String(purchase.id))}>Refresh status</Button>}</div></div>{labels.map((label) => <p key={String(label.id)} className="mt-2 text-muted-foreground">{String((label.tracking as Record<string, unknown> | undefined)?.company || "Shopify Shipping")} / {String((label.tracking as Record<string, unknown> | undefined)?.number || "Tracking pending")}</p>)}{errors.map((error, index) => <p key={`${String(error.code)}-${index}`} className="mt-2 text-destructive">{String(error.message || "Shopify could not purchase the label.")}</p>)}</div>})}</CardContent></Card>
}
