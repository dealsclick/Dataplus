...
function OrderDetailWorkspace() {
  ...
  const hasPurchaseOrders = purchaseOrderIds.length > 0 || purchaseOrderNumbers.length > 0 || purchaseRoutes.some((route) => Boolean(route.purchaseOrderId))
  const shopifyAdminUrl = String(order?.shopifyAdminUrl || "")
  ...
  return <div className="grid gap-5">
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(280px,0.9fr)]"><Tabs defaultValue="items"><TabsList className="flex flex-wrap"><TabsTrigger value="items">Items &amp; P&amp;L</TabsTrigger><TabsTrigger value="fulfillment">Fulfillment ({shipments.length})</TabsTrigger><TabsTrigger value="purchase-orders">POs ({Math.max(purchaseOrderIds.length, purchaseOrderNumbers.length)})</TabsTrigger><TabsTrigger value="finance">Finance</TabsTrigger><TabsTrigger value="customer">Customer</TabsTrigger><TabsTrigger value="returns">Returns ({relatedReturns.length})</TabsTrigger><TabsTrigger value="documents">Documents ({documents.length})</TabsTrigger><TabsTrigger value="channel">Channel</TabsTrigger><TabsTrigger value="activity">Activity ({events.length})</TabsTrigger></TabsList>
      ...
      <TabsContent value="purchase-orders" className="grid gap-4 pt-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-medium">Supplier purchase orders</p><p className="text-sm text-muted-foreground">Purchase orders are split by supplier and remain linked to this customer order.</p></div><Button size="sm" disabled={saving} onClick={() => void createPurchaseOrders()}><Boxes className="size-4" /> Create supplier PO(s)</Button></div>{hasPurchaseOrders ? <div className="grid gap-3">{[...new Set([...purchaseOrderIds, ...purchaseRoutes.map((route) => String(route.purchaseOrderId || "")).filter(Boolean)])].map((poId, index) => <Card key={poId}><CardContent className="flex flex-wrap items-center justify-between gap-3 p-4 text-sm"><div><p className="font-medium">{purchaseOrderNumbers[index] || `Purchase order ${index + 1}`}</p><p className="text-muted-foreground">{String(purchaseRoutes.find((route) => String(route.purchaseOrderId || "") === poId)?.supplierName || purchaseRoutes.find((route) => String(route.purchaseOrderId || "") === poId)?.supplier || "Supplier purchase order")}</p></div><Button size="sm" variant="outline" asChild><a href={`/purchase-orders/${encodeURIComponent(poId)}`}>Open PO</a></Button></CardContent></Card>)}</div> : <Card><CardContent className="flex flex-wrap items-center justify-between gap-3 p-5 text-sm text-muted-foreground"><span>No purchase order is linked to this order.</span><Button size="sm" disabled={saving} onClick={() => void createPurchaseOrders()}>Create supplier PO(s)</Button></CardContent></Card>}</TabsContent>
