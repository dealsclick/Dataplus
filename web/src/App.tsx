...
function ShippingLabelReadinessPanel({ orderId, order, onUpdated }: { orderId: string; order: Record<string, unknown>; onUpdated: () => Promise<void> }) {
  ...
  const latestPackage = (purchases[0]?.package || {}) as Record<string, unknown>
  const [draft, setDraft] = useState({ weightPounds: String(latestPackage.weightPounds || ""), lengthInches: String(latestPackage.lengthInches || ""), widthInches: String(latestPackage.widthInches || ""), heightInches: String(latestPackage.heightInches || ""), packageType: String(latestPackage.packageType || "BOX"), shipDate: String(latestPackage.shipDate || new Date().toISOString().slice(0, 10)), carrierCode: "", serviceCode: "", notifyCustomer: false })
  const check = async () => { setLoading(true); try { const next = await api<{ ready?: boolean; blockers?: string[]; apiVersion?: string }>(`/api/orders/${encodeURIComponent(orderId)}/shipping-label-readiness`); setResult(next); return next } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to check label readiness."); return null } finally { setLoading(false) } }
  const begin = async () => { const next = await check(); if (next?.ready) setOpen(true) }
  const purchase = async () => { setLoading(true); try { const response = await api<{ message?: string }>(`/api/orders/${encodeURIComponent(orderId)}/shipping-labels/shopify`, { method: "POST", body: JSON.stringify({ ...draft, weightPounds: Number(draft.weightPounds || 0), lengthInches: Number(draft.lengthInches || 0), widthInches: Number(draft.widthInches || 0), heightInches: Number(draft.heightInches || 0) }) }); setOpen(false); await onUpdated(); toast.success(response.message || "Shopify label purchase started.") } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to purchase Shopify label.") } finally { setLoading(false) } }
  ...
  return <Card><CardHeader><CardTitle className="text-sm">Shopify Shipping labels</CardTitle><CardDescription>Purchase one label for an eligible Shopify fulfillment order. Shopify calculates the rate; a label is charged only after you confirm the purchase.</CardDescription></CardHeader><CardContent className="grid gap-3"><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => void check()} disabled={loading}>{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />} Check readiness</Button><Button size="sm" onClick={() => void begin()} disabled={loading || !result?.ready}>Purchase label</Button></div>{result && <div className="rounded-md border p-3 text-sm"><Badge variant={result.ready ? "default" : "outline"}>{result.ready ? "Ready for label purchase" : "Needs setup"}</Badge><p className="mt-2 text-muted-foreground">Shopify API: {result.apiVersion || "Not configured"}</p>{!result.ready && <ul className="mt-2 list-disc space-y-1 pl-5 text-muted-foreground">{(result.blockers || []).map((blocker) => <li key={blocker}>{blocker}</li>)}</ul>}</div>}{purchases.map((purchase) => { const labels = Array.isArray(purchase.labels) ? purchase.labels as Array<Record<string, unknown>> : []; const errors = Array.isArray(purchase.errors) ? purchase.errors as Array<Record<string, unknown>> : []; return <div key={String(purchase.id)} className="rounded-md border p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-medium">Shopify label purchase</p><p className="text-xs text-muted-foreground">{dateLabel(String(purchase.requestedAt || purchase.checkedAt || ""))}</p></div><div className="flex items-center gap-2"><Badge variant={String(purchase.status) === "PURCHASED" ? "default" : String(purchase.status) === "PURCHASE_FAILED" ? "destructive" : "secondary"}>{String(purchase.status || "PENDING_PURCHASE")}</Badge>{!Boolean(purchase.done) && <Button size="sm" variant="outline" disabled={loading} onClick={() => void refreshPurchase(String(purchase.id))}>Refresh status</Button>}</div></div>{labels.map((label) => <p key={String(label.id)} className="mt-2 text-muted-foreground">{String((label.tracking as Record<string, unknown> | undefined)?.company || "Shopify Shipping")} / {String((label.tracking as Record<string, unknown> | undefined)?.number || "Tracking pending")}</p>)}{errors.map((error, index) => <p key={`${String(error.code)}-${index}`} className="mt-2 text-destructive">{String(error.message || "Shopify could not purchase the label.")}</p>)}</div>})}<Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Purchase Shopify Shipping label</DialogTitle><DialogDescription>This is a paid Shopify Shipping action. Enter the actual packed package, then confirm. Leave the carrier preference blank to let Shopify select the best available rate.</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><Field label="Weight (lb)"><Input type="number" min="0.01" step="0.01" value={draft.weightPounds} onChange={(event) => setDraft({ ...draft, weightPounds: event.target.value })} /></Field><Field label="Package type"><Select value={draft.packageType} onValueChange={(packageType) => setDraft({ ...draft, packageType })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="BOX">Box</SelectItem><SelectItem value="ENVELOPE">Envelope</SelectItem><SelectItem value="SOFT_PACK">Soft pack</SelectItem><SelectItem value="FLAT_RATE">Flat rate</SelectItem></SelectContent></Select></Field><Field label="Length (in)"><Input type="number" min="0.01" step="0.01" value={draft.lengthInches} onChange={(event) => setDraft({ ...draft, lengthInches: event.target.value })} /></Field><Field label="Width (in)"><Input type="number" min="0.01" step="0.01" value={draft.widthInches} onChange={(event) => setDraft({ ...draft, widthInches: event.target.value })} /></Field><Field label="Height (in)"><Input type="number" min="0.01" step="0.01" value={draft.heightInches} onChange={(event) => setDraft({ ...draft, heightInches: event.target.value })} /></Field><Field label="Ship date"><Input type="date" value={draft.shipDate} onChange={(event) => setDraft({ ...draft, shipDate: event.target.value })} /></Field><Field label="Carrier code (optional)"><Input value={draft.carrierCode} placeholder="usps" onChange={(event) => setDraft({ ...draft, carrierCode: event.target.value })} /></Field><Field label="Service code (optional)"><Input value={draft.serviceCode} placeholder="ground_advantage" onChange={(event) => setDraft({ ...draft, serviceCode: event.target.value })} /></Field><div className="sm:col-span-2"><ToggleField label="Notify customer after purchase" checked={draft.notifyCustomer} onCheckedChange={(notifyCustomer) => setDraft({ ...draft, notifyCustomer })} /></div></div><DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button disabled={loading || !Number(draft.weightPounds) || !Number(draft.lengthInches) || !Number(draft.widthInches) || !Number(draft.heightInches)} onClick={() => void purchase()}>Confirm and purchase label</Button></DialogFooter></DialogContent></Dialog></CardContent></Card>
}
  ...
  return <div className="grid gap-5">
    ...
    <Tabs defaultValue="items"><TabsList className="flex flex-wrap"><TabsTrigger value="items">Items & P&amp;L</TabsTrigger><TabsTrigger value="fulfillment">Fulfillment ({shipments.length})</TabsTrigger><TabsTrigger value="finance">Finance</TabsTrigger><TabsTrigger value="customer">Customer</TabsTrigger><TabsTrigger value="returns">Returns ({relatedReturns.length})</TabsTrigger><TabsTrigger value="documents">Documents ({documents.length})</TabsTrigger><TabsTrigger value="channel">Channel</TabsTrigger><TabsTrigger value="activity">Activity ({events.length})</TabsTrigger></TabsList>
      ...
      <TabsContent value="fulfillment" className="grid gap-4 pt-4">
        ...
        <ShippingLabelReadinessPanel orderId={orderId} order={order} onUpdated={load} />
