...
function CategoriesWorkspace({ categoryId = "", standalone = false, initialScope = "main" }: { categoryId?: string; standalone?: boolean; initialScope?: "main" | "source" }) {
  ...
  const visibleCategories = useMemo(() => {
    ...
    return categories.filter((row) => {
      ...
      const googleMapped = Boolean(row.mappings?.shopify?.googleCategory?.id || row.mappings?.shopify?.googleCategory?.breadcrumb || row.mappings?.shopify?.googleCategory?.fullName)
      ...
      if (mappingFilter === "fully-mapped" && !(shopifyMapped && googleMapped && shopifyCollectionMapped && ebayMapped)) return false
      if (mappingFilter === "missing-shopify" && shopifyMapped) return false
      if (mappingFilter === "missing-google" && googleMapped) return false
