...
function PurchaseOrderDetailPage() {
  ...
  const receiveAll = async () => { setSaving(true); try { await api(`/api/purchase-orders/${encodeURIComponent(id)}/receive`, { method: "POST", body: JSON.stringify({ user: "Luis", mode: "final", note: receiptNote, items: lines.map((line) => ({ sku: line.sku, qtyReceived: Math.max(0, Number(line.remainingQty ?? Number(line.qty || 0) - Number(line.receivedQty || 0))) })) }) }); toast.success("Receipt saved and linked order routes released."); setReceivingOpen(false); setReceiptNote(""); await load() } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to receive purchase order.") } finally { setSaving(false) } }
  ...
  return <div className="grid gap-5">
    ...
    <Dialog open={receivingOpen} onOpenChange={setReceivingOpen}><DialogContent className="max-w-3xl"><DialogHeader><DialogTitle>Receive {String(po.poNumber || "purchase order")}</DialogTitle><DialogDescription>Receive all remaining units into {String(po.warehouseName || "the destination warehouse")}. Partial receiving is tracked per PO line and releases linked orders for fulfillment.</DialogDescription></DialogHeader><div className="max-h-72 overflow-auto rounded-md border"><Table><TableHeader><TableRow><TableHead>SKU</TableHead><TableHead>Remaining</TableHead><TableHead>Receive now</TableHead></TableRow></TableHeader><TableBody>{lines.map((line, index) => <TableRow key={`receive-${index}`}><TableCell>{String(line.sku || "-")}</TableCell><TableCell>{numberLabel(Number(line.remainingQty ?? Math.max(0, Number(line.qty || 0) - Number(line.receivedQty || 0))))}</TableCell><TableCell>{numberLabel(Number(line.remainingQty ?? Math.max(0, Number(line.qty || 0) - Number(line.receivedQty || 0))))}</TableCell></TableRow>)}</TableBody></Table></div><div className="grid gap-2"><Label htmlFor="receipt-note">Receiving note</Label><Textarea id="receipt-note" value={receiptNote} onChange={(event) => setReceiptNote(event.target.value)} placeholder="Packing slip, variance, bin, or receiving notes" /></div><DialogFooter><Button variant="outline" onClick={() => setReceivingOpen(false)}>Cancel</Button><Button disabled={saving} onClick={() => void receiveAll()}>{saving ? <Loader2 className="size-4 animate-spin" /> : "Receive remaining units"}</Button></DialogFooter></DialogContent></Dialog>
  </div>
}
