...
function ShippingLabelReadinessPanel({ orderId, order, onUpdated }: { orderId: string; order: Record<string, unknown>; onUpdated: () => Promise<void> }) {
  const [loading, setLoading] = useState(false)
  ...
  const check = async () => { setLoading(true); try { setResult(await api(`/api/orders/${encodeURIComponent(orderId)}/shipping-label-readiness`)) } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to check label readiness.") } finally { setLoading(false) } }
