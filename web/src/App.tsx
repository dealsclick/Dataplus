...
function OperationsPage() {
  ...
  async function load() {
    setLoading(true)
    try { setData(await api("/api/orders?limit=5000")) }
  ...
  return <div className="grid gap-5">
    <PageHeader eyebrow="Operations" title={tab === "orders" ? "Orders" : tab === "drafts" ? "Draft orders" : "Returns"} description="Work orders by operational state, then open the full record for fulfillment, payments, and channel actions." action={<div className="flex gap-2"><Button size="sm" variant="outline" disabled={busy || loading} onClick={() => void importShopifyOrders()}><Store className="size-4" /> Import Shopify orders</Button><Button size="sm" variant="outline" disabled={loading} onClick={() => void load()}>{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />} Refresh</Button></div>} />
    ...
    <Card>
      ...
      <CardContent className="p-0">
        {loading ? <div className="grid gap-2 p-4"><Skeleton className="h-12" /><Skeleton className="h-12" /><Skeleton className="h-12" /></div> : <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead className="w-10"><Checkbox aria-label="Select visible orders" checked={filtered.length > 0 && selectedVisible.length === filtered.length} onCheckedChange={(checked) => setSelectedIds(checked === true ? new Set(filtered.map((row) => String(row.id || ""))) : new Set())} /></TableHead><TableHead>Order</TableHead><TableHead>Customer</TableHead><TableHead>Delivery</TableHead><TableHead>Items</TableHead><TableHead>Total</TableHead><TableHead>Payment</TableHead><TableHead>Supply</TableHead><TableHead>PO</TableHead><TableHead>Channel</TableHead><TableHead>Updated</TableHead><TableHead /></TableRow></TableHeader><TableBody>{filtered.map((row, index) => { const reference = String(row.orderNumber || row.reference || row.returnNumber || row.id || "-"); const items = Array.isArray(row.items) ? row.items as Array<Record<string, unknown>> : []; const itemCount = items.reduce((total, item) => total + Number(item.qty || 0), 0) || Number(row.qty || 0); const id = String(row.id || ""); const orderQueue = tab === "orders" ? queueFor(row) : String(row.status || row.returnStatus || "draft"); const hasPo = (Array.isArray(row.purchaseOrderIds) && row.purchaseOrderIds.length > 0) || (Array.isArray(row.purchaseOrderNumbers) && row.purchaseOrderNumbers.length > 0) || (Array.isArray(row.fulfillmentRoutes) && (row.fulfillmentRoutes as Array<Record<string, unknown>>).some((route) => Boolean(route.purchaseOrderId))); return <TableRow key={`${reference}-${index}`} data-state={selectedIds.has(id) ? "selected" : undefined}><TableCell><Checkbox aria-label={`Select ${reference}`} checked={selectedIds.has(id)} onCheckedChange={(checked) => setSelectedIds((current) => { const next = new Set(current); if (checked === true) next.add(id); else next.delete(id); return next })} /></TableCell><TableCell className="min-w-44"><div className="group flex items-center gap-1"><a href={`/orders/${encodeURIComponent(id || reference)}`} className="font-medium hover:underline">{reference}</a><Button size="icon" variant="ghost" className="size-7 opacity-0 group-hover:opacity-100 focus-visible:opacity-100" title="Quick view" onClick={() => void openOrder(row)}><PackageSearch className="size-4" /></Button></div><p className="text-xs text-muted-foreground">{String(row.source || "Manual")} / {String(row.channelSource || row.salesChannel || "-")}</p></TableCell><TableCell className="min-w-40">{String(row.buyer || row.customerName || row.customerEmail || "-")}</TableCell><TableCell>{String(row.shipBy || "-")}</TableCell><TableCell>{numberLabel(itemCount)}</TableCell><TableCell className="font-medium">{moneyLabel(Number(row.total || row.refundAmount || 0))}</TableCell><TableCell><Badge variant={String(row.financialStatus || "").toLowerCase() === "paid" ? "secondary" : "outline"}>{String(row.financialStatus || row.paymentStatus || "-")}</Badge></TableCell><TableCell><Badge variant={orderQueue === "review" ? "destructive" : orderQueue === "ready-ship" ? "secondary" : "outline"}>{orderQueue.replace(/-/g, " ")}</Badge></TableCell><TableCell><span className={`inline-flex items-center gap-1 text-xs font-medium ${hasPo ? "text-emerald-700" : "text-muted-foreground"}`} title={hasPo ? `Purchase order: ${Array.isArray(row.purchaseOrderNumbers) ? row.purchaseOrderNumbers.join(", ") : "linked"}` : "No purchase order linked"}>{hasPo ? <CheckCircle2 className="size-4" /> : <AlertCircle className="size-4" />}{hasPo ? "Linked" : "None"}</span></TableCell><TableCell>{String(row.source || "-")}</TableCell><TableCell>{dateLabel(String(row.updatedAt || row.createdAt || ""))}</TableCell><TableCell><DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" variant="ghost" title="Order actions"><MoreHorizontal className="size-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={() => void openOrder(row)}>Quick view</DropdownMenuItem><DropdownMenuItem asChild><a href={`/orders/${encodeURIComponent(id || reference)}`}>Open order</a></DropdownMenuItem>{tab === "orders" && <><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => void runAction([id], "approve")}>Approve</DropdownMenuItem><DropdownMenuItem onSelect={() => void runAction([id], "hold")}>Put on hold</DropdownMenuItem><DropdownMenuItem onSelect={() => void createSupplierPos([id])}>Create supplier PO(s)</DropdownMenuItem><DropdownMenuItem onSelect={() => void createBackorder(row)}>Create backorder PO</DropdownMenuItem><DropdownMenuItem onSelect={() => void runAction([id], "done")}>Mark done</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => void runAction([id], "cancel")}>Cancel locally</DropdownMenuItem><DropdownMenuItem className="text-destructive focus:text-destructive" onSelect={() => void runAction([id], "void")}>Void locally</DropdownMenuItem></>}</DropdownMenuContent></DropdownMenu></TableCell></TableRow> })}{!filtered.length && <TableRow><TableCell colSpan={12} className="h-28 text-center text-muted-foreground">No records match the current queue and filters.</TableCell></TableRow>}</TableBody></Table></div>}
      </CardContent>
    </Card>
