...
function PurchasingPage() {
  ...
  return <div className="grid gap-5">
    ...
    <div className="grid gap-3 sm:grid-cols-3"><Detail label="Open requirements" value={numberLabel(openRequirements)} /><Detail label="Buyer review" value={numberLabel(requirements.filter((row) => ["new", "buyer_review", "awaiting_approval"].includes(String(row.status))).length)} /><Detail label="Open POs" value={numberLabel(pos.filter((row) => !["received", "closed", "canceled"].includes(String(row.status))).length)} /></div>
    <Tabs value={tab} onValueChange={setTab}>
      <TabsList><TabsTrigger value="requirements">Purchase requirements ({numberLabel(requirements.length)})</TabsTrigger><TabsTrigger value="pos">Purchase orders ({numberLabel(pos.length)})</TabsTrigger><TabsTrigger value="risks">Exceptions & risk ({numberLabel(risks.length)})</TabsTrigger></TabsList>
