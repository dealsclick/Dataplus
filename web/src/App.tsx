...
function OrderNotesPanel({ orderId, order, onUpdated }: { orderId: string; order: Record<string, unknown>; onUpdated: () => Promise<void> }) {
  ...
  const save = async () => {
    if (!text.trim()) return toast.error("Enter a note before saving.")
    setSaving(true)
    try {
      await api(`/api/orders/${encodeURIComponent(orderId)}/notes`, { method: "POST", body: JSON.stringify({ text: text.trim(), user: "Luis" }) })
      setText("")
      await onUpdated()
      toast.success("Internal order note added.")
    } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to save order note.") } finally { setSaving(false) }
  }
  return <div className="grid gap-4"><Card><CardHeader><CardTitle className="text-sm">Add internal note</CardTitle><CardDescription>Notes are visible to the DataPlus team and every entry is recorded in the Activity log.</CardDescription></CardHeader><CardContent className="grid gap-3"><Textarea className="min-h-28" value={text} onChange={(event) => setText(event.target.value)} placeholder="Add purchasing, fulfillment, customer-service, or follow-up context..." /><div className="flex justify-end"><Button size="sm" disabled={saving || !text.trim()} onClick={() => void save()}>{saving && <Loader2 className="size-4 animate-spin" />} Save note</Button></div></CardContent></Card><Card><CardHeader><CardTitle className="text-sm">Internal notes</CardTitle><CardDescription>{notes.length ? `${numberLabel(notes.length)} note${notes.length === 1 ? "" : "s"} on this order.` : "No standalone internal notes yet."}</CardDescription></CardHeader><CardContent className="grid gap-3">{notes.map((note) => <div key={String(note.id)} className="rounded-md border p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-medium">{String(note.createdBy || "DataPlus user")}</span><span className="text-xs text-muted-foreground">{dateLabel(String(note.createdAt || ""))}</span></div><p className="mt-2 whitespace-pre-wrap text-muted-foreground">{String(note.text || "")}</p></div>)}{!notes.length && <p className="text-sm text-muted-foreground">Use the form above to add the first internal note.</p>}</CardContent></Card></div>
}
