  ...
  function queueShopifyAction(kind: string, apply = false) {
    ...
    const actionMap: Record<string, { path: string; body: Record<string, unknown>; confirmMessage?: string; successMessage: string }> = {
      ...
      orderImport: {
        ...
        body: { limit: Number(settings.shopifyOrderImportLimit || 250), sources: String(settings.shopifyOrderImportSources || "All Shopify sources"), includeCanceled: Boolean(settings.shopifyOrderImportIncludeCanceled) },
          ...
          <Card>
            ...
            <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              ...
              {isShopify && <>
                ...
                <Field label="Order source allowlist"><Input disabled={!editing} value={String(settings.shopifyOrderImportSources || "All Shopify sources")} placeholder="All Shopify sources" onChange={(event) => update("shopifyOrderImportSources", event.target.value)} /><p className="text-xs text-muted-foreground">Use “All Shopify sources” to import Online Store, Shop, Point of Sale, completed draft orders, mobile, API, and future channel sources. Enter a comma-separated list to restrict imports.</p></Field>
