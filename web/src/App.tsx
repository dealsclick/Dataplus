...
function OperationsPage() {
  ...
  return <div className="grid gap-5">
    ...
    {tab === "drafts" && !loading && filtered.length > 0 && <Card><CardHeader className="pb-3"><CardTitle className="text-sm">Open a quote</CardTitle><CardDescription>Select a quote ID to review its customer, complete ship-to address, lines, pricing, notes, and revision history.</CardDescription></CardHeader><CardContent className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{filtered.slice(0, 12).map((draft) => <a key={String(draft.id)} href={`/drafts/${encodeURIComponent(String(draft.id || draft.draftNumber || ""))}`} className="rounded-md border p-3 transition-colors hover:bg-muted/50"><div className="flex items-center justify-between gap-2"><span className="font-medium hover:underline">{String(draft.draftNumber || draft.id)}</span><Badge variant="outline">{String(draft.status || "draft")}</Badge></div><p className="mt-1 truncate text-sm text-muted-foreground">{String(draft.buyer || draft.buyerEmail || "No customer")}</p><p className="mt-1 text-xs text-muted-foreground">{moneyLabel((Array.isArray(draft.items) ? draft.items as Array<Record<string, unknown>> : []).reduce((total, item) => total + Number(item.price || 0) * Number(item.qty || 0), 0))}</p></a>)}</CardContent></Card>}
