...
function PurchasingPage() {
  ...
  const supplierPerformance: any[] = Object.values(pos.reduce((groups, po) => { const key = String(po.vendorId || po.supplier || "unassigned"); const group = groups[key] || { vendorId: String(po.vendorId || ""), supplier: String(po.supplier || "Unassigned"), total: 0, open: 0, received: 0, acknowledged: 0, overdue: 0, commitment: 0, units: 0, receivedUnits: 0 }; group.total += 1; group.units += Number(po.totalUnits || 0); group.receivedUnits += (Array.isArray(po.items) ? po.items as Array<Record<string, unknown>> : []).reduce((sum, line) => sum + Number(line.receivedQty || 0), 0); group.commitment += Number(po.estimatedCost || 0); const status = String(po.status || "").toLowerCase(); if (!["received", "closed", "canceled", "rejected"].includes(status)) group.open += 1; if (["received", "closed"].includes(status)) group.received += 1; if (po.vendorAcknowledgement || ["vendor_confirmed", "partially_received", "received", "closed"].includes(status)) group.acknowledged += 1; if (!["received", "closed", "canceled", "rejected"].includes(status) && String(po.expectedAt || "") && String(po.expectedAt) < today) group.overdue += 1; groups[key] = group; return groups }, {} as Record<string, { vendorId: string; supplier: string; total: number; open: number; received: number; acknowledged: number; overdue: number; commitment: number; units: number; receivedUnits: number }>))
