...
function WarehouseBinManager() {
  type Bin = { id?: string; code?: string; name?: string; type?: string; section?: string; aisle?: string; active?: boolean; isDefault?: boolean; notes?: string }
  ...
  const [form, setForm] = useState({ code: "", name: "", type: "Storage", section: "Storage", aisle: "", notes: "", active: true, isDefault: false })
  const [busy, setBusy] = useState(false)
  ...
  useEffect(() => { void load() }, [])
  const openCreate = () => { setEditing(null); setForm({ code: "", name: "", type: "Storage", section: "Storage", aisle: "", notes: "", active: true, isDefault: !(selected?.bins || []).length }); setDialogOpen(true) }
  const openEdit = (bin: Bin) => { setEditing(bin); setForm({ code: String(bin.code || ""), name: String(bin.name || ""), type: String(bin.type || "Storage"), section: String(bin.section || "Storage"), aisle: String(bin.aisle || ""), notes: String(bin.notes || ""), active: bin.active !== false, isDefault: bin.isDefault === true }); setDialogOpen(true) }
  const save = async () => {
