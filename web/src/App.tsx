...
function PurchasingPage() {
  ...
  return <div className="grid gap-5">
    ...
    <Tabs value={tab} onValueChange={setTab}>
      <TabsList><TabsTrigger value="requirements">Purchase requirements ({numberLabel(requirements.length)})</TabsTrigger><TabsTrigger value="pos">Purchase orders ({numberLabel(pos.length)})</TabsTrigger></TabsList>
...
function PurchaseOrderDetailPage() {
  ...
  return <div className="grid gap-5">
    <PageHeader eyebrow="Purchasing / Purchase order" title={String(po.poNumber || id)} description={`${String(po.supplier || "Unassigned supplier")} to ${String(po.warehouseName || "unassigned destination")}`} action={<div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" asChild><a href="/purchasing">Back</a></Button><Button size="sm" variant="outline" onClick={() => void load()}><RefreshCw className="size-4" /> Refresh</Button><DropdownMenu><DropdownMenuTrigger asChild><Button size="sm" disabled={saving}>Actions</Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={openEditor}>Edit PO</DropdownMenuItem><DropdownMenuItem onClick={() => void updateStatus("approve")}>Approve PO</DropdownMenuItem><DropdownMenuItem onClick={() => void submit()}>Submit to supplier</DropdownMenuItem><DropdownMenuItem onClick={openReceiving}>Receive inventory</DropdownMenuItem><DropdownMenuItem onClick={() => window.open(`/api/purchase-orders/${encodeURIComponent(id)}/export?format=csv`, "_blank", "noreferrer")}>Export CSV</DropdownMenuItem><DropdownMenuItem onClick={() => window.open(`/api/purchase-orders/${encodeURIComponent(id)}/export?format=html`, "_blank", "noreferrer")}>Print PO</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={() => void updateStatus("hold")}>Place on hold</DropdownMenuItem><DropdownMenuItem onClick={() => void updateStatus("close")}>Close PO</DropdownMenuItem><DropdownMenuItem className="text-destructive" onClick={() => void updateStatus("cancel")}>Cancel PO</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>} />
