...
function OrderDocumentsPanel({ orderId, documents, onUpdated }: { orderId: string; documents: Array<Record<string, unknown>>; onUpdated: () => Promise<void> }) {
  const [open, setOpen] = useState(false); const [saving, setSaving] = useState(false); const [draft, setDraft] = useState({ name: "", url: "", type: "other", note: "" })
  const save = async () => { setSaving(true); try { await api(`/api/orders/${encodeURIComponent(orderId)}/documents`, { method: "POST", body: JSON.stringify(draft) }); setOpen(false); setDraft({ name: "", url: "", type: "other", note: "" }); await onUpdated(); toast.success("Document linked to order.") } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to link document.") } finally { setSaving(false) } }
  return <div className="grid gap-4"><div><Button onClick={() => setOpen(true)}>Link document</Button></div>{documents.length ? <div className="grid gap-2">{documents.map((document) => <a key={String(document.id)} href={String(document.url || "#")} target="_blank" rel="noreferrer" className="flex flex-wrap items-center justify-between gap-2 rounded-md border p-3 text-sm hover:bg-muted/50"><span className="font-medium">{String(document.name || "Document")}</span><span className="text-muted-foreground">{String(document.type || "other")} / {dateLabel(String(document.createdAt || ""))}</span></a>)}</div> : <Card><CardContent className="p-5 text-sm text-muted-foreground">No documents are linked to this order.</CardContent></Card>}<Dialog open={open} onOpenChange={setOpen}><DialogContent><DialogHeader><DialogTitle>Link order document</DialogTitle><DialogDescription>Link an invoice, packing slip, vendor confirmation, or other document stored elsewhere.</DialogDescription></DialogHeader><div className="grid gap-4"><Field label="Name"><Input value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></Field><Field label="URL"><Input value={draft.url} onChange={(event) => setDraft({ ...draft, url: event.target.value })} placeholder="https://..." /></Field><Field label="Type"><Select value={draft.type} onValueChange={(type) => setDraft({ ...draft, type })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="invoice">Invoice</SelectItem><SelectItem value="packing-slip">Packing slip</SelectItem><SelectItem value="label">Shipping label</SelectItem><SelectItem value="other">Other</SelectItem></SelectContent></Select></Field><Field label="Note"><Textarea value={draft.note} onChange={(event) => setDraft({ ...draft, note: event.target.value })} /></Field></div><DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button disabled={saving} onClick={() => void save()}>Link document</Button></DialogFooter></DialogContent></Dialog></div>
}
...
function OrderDetailWorkspace() {
  ...
  return <div className="grid gap-5">
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_260px]"><Card><CardHeader className="gap-3 border-b"><div className="flex flex-wrap items-center gap-2"><Badge variant="outline">{String(order.fulfillmentWarehouseName || "Staten Island warehouse")}</Badge></div><div className="rounded-md border bg-muted/20 p-3 text-sm"><p className="font-medium">{String(order.shippingService || "Shipping service will be selected with the label")}</p><p className="mt-1 text-muted-foreground">{String(order.shippingAddressLabel || "Delivery address")}</p></div></CardHeader><CardContent className="grid gap-3 p-3"><OrderFulfillmentItemRows lines={lines} remaining={remaining} /><div className="flex flex-wrap justify-end gap-2 border-t pt-3"><Button size="sm" variant="outline" onClick={() => openFulfill(true)}>Mark as fulfilled</Button>{String(order.source || "").toLowerCase() === "shopify" && <Button size="sm" onClick={openShippingLabel}><Truck className="size-4" /> Create shipping label</Button>}</div></CardContent></Card><div className="grid content-start gap-4"><Card><CardHeader className="pb-2"><CardTitle className="text-sm">Customer</CardTitle><CardDescription>{String(order.buyer || order.buyerEmail || "Customer")}</CardDescription></CardHeader><CardContent className="grid gap-3 text-sm"><div><p className="font-medium">Contact information</p><p className="mt-1 break-words text-muted-foreground">{String(order.buyerEmail || "No email")}</p><p className="text-muted-foreground">{String(order.phone || "No phone")}</p></div><div><p className="font-medium">Shipping address</p><p className="mt-1 whitespace-pre-line text-muted-foreground">{addressText(order.address)}</p></div></CardContent></Card><Card><CardHeader className="pb-2"><CardTitle className="text-sm">Payment</CardTitle></CardHeader><CardContent className="grid gap-2 text-sm"><div className="flex justify-between"><span>Items</span><span>{moneyLabel(Number(pnl.itemRevenue || order.total || 0))}</span></div><div className="flex justify-between"><span>Shipping</span><span>{moneyLabel(Number(pnl.shippingCollected || 0))}</span></div><div className="flex justify-between border-t pt-2 font-medium"><span>Total</span><span>{moneyLabel(Number(order.total || 0))}</span></div></CardContent></Card></div></div>
    <div className="grid gap-4"><Tabs defaultValue="items"><TabsList className="flex flex-wrap"><TabsTrigger value="items">Profit &amp; Loss</TabsTrigger><TabsTrigger value="fulfillment">Fulfillment ({shipments.length})</TabsTrigger><TabsTrigger value="purchase-orders">POs ({Math.max(purchaseOrderIds.length, purchaseOrderNumbers.length)})</TabsTrigger><TabsTrigger value="finance">Finance</TabsTrigger><TabsTrigger value="customer">Customer</TabsTrigger><TabsTrigger value="returns">Returns ({relatedReturns.length})</TabsTrigger><TabsTrigger value="documents">Documents ({documents.length})</TabsTrigger><TabsTrigger value="channel">Channel</TabsTrigger><TabsTrigger value="activity">Activity ({events.length})</TabsTrigger></TabsList>
...
function OperationsPage() {
  ...
  async function openOrder(row: Record<string, unknown>) {
    ...
    try {
      const result = await api<{ order?: Record<string, unknown> }>(`/api/orders/${encodeURIComponent(String(row.id || row.orderNumber || ""))}`)
      setDetail(result.order || row)
    } catch { setDetail(row) }
  }
  ...
  return <div className="grid gap-5">
    ...
    <Card>
      ...
      <CardContent className="p-0">
        ...
        {tab !== "drafts" && <>
        {loading ? <div className="grid gap-2 p-4"><Skeleton className="h-12" /><Skeleton className="h-12" /><Skeleton className="h-12" /></div> : <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead className="w-10"><Checkbox aria-label="Select visible records" checked={filtered.length > 0 && selectedVisible.length === filtered.length} onCheckedChange={(checked) => setSelectedIds(checked === true ? new Set(filtered.map((row) => String(row.id || ""))) : new Set())} /></TableHead><TableHead>{tab === "orders" ? "Order" : tab === "drafts" ? "Quote" : "Return"}</TableHead><TableHead>Customer</TableHead><TableHead>{tab === "returns" ? "Warehouse" : tab === "drafts" ? "Items" : "Delivery"}</TableHead><TableHead>{tab === "returns" ? "Reason" : "Items"}</TableHead><TableHead>{tab === "returns" ? "Amount" : "Total"}</TableHead><TableHead>{tab === "orders" ? "Payment" : "Status"}</TableHead><TableHead>{tab === "returns" ? "Disposition" : tab === "drafts" ? "Source" : "Supply"}</TableHead><TableHead>{tab === "orders" ? "PO" : "Channel sync"}</TableHead><TableHead>Channel</TableHead><TableHead>Updated</TableHead><TableHead /></TableRow></TableHeader><TableBody>{filtered.map((row, index) => { const reference = String(row.orderNumber || row.draftNumber || row.returnNumber || row.reference || row.id || "-"); const items = Array.isArray(row.items) ? row.items as Array<Record<string, unknown>> : []; const itemCount = items.reduce((total, item) => total + Number(item.qty || 0), 0) || Number(row.qty || 0); const id = String(row.id || ""); const orderQueue = tab === "orders" ? queueFor(row) : String(row.status || row.returnStatus || "draft"); const hasPo = (Array.isArray(row.purchaseOrderIds) && row.purchaseOrderIds.length > 0) || (Array.isArray(row.purchaseOrderNumbers) && row.purchaseOrderNumbers.length > 0) || (Array.isArray(row.fulfillmentRoutes) && (row.fulfillmentRoutes as Array<Record<string, unknown>>).some((route) => Boolean(route.purchaseOrderId))); const linkedOrderId = String(row.orderId || ""); return <TableRow key={`${reference}-${index}`} data-state={selectedIds.has(id) ? "selected" : undefined}><TableCell><Checkbox aria-label={`Select ${reference}`} checked={selectedIds.has(id)} onCheckedChange={(checked) => setSelectedIds((current) => { const next = new Set(current); if (checked === true) next.add(id); else next.delete(id); return next })} /></TableCell><TableCell className="min-w-44"><div className="group flex items-center gap-1">{tab === "orders" ? <><a href={`/orders/${encodeURIComponent(id || reference)}`} className="font-medium hover:underline">{reference}</a><Button size="icon" variant="ghost" className="size-7 opacity-0 group-hover:opacity-100 focus-visible:opacity-100" title="Quick view" onClick={() => void openOrder(row)}><PackageSearch className="size-4" /></Button></> : <span className="font-medium">{reference}</span>}</div><p className="text-xs text-muted-foreground">{tab === "returns" ? `Order ${String(row.orderNumber || "-")}` : String(row.source || "Manual")}</p></TableCell><TableCell className="min-w-40">{String(row.buyer || row.customerName || row.customerEmail || "-")}</TableCell><TableCell>{tab === "returns" ? String(row.warehouseName || "-") : tab === "drafts" ? numberLabel(itemCount) : String(row.shipBy || "-")}</TableCell><TableCell>{tab === "returns" ? String(row.reason || "-") : numberLabel(itemCount)}</TableCell><TableCell className="font-medium">{moneyLabel(Number(row.total || row.amount || row.refundAmount || 0))}</TableCell><TableCell><Badge variant={String(row.financialStatus || "").toLowerCase() === "paid" ? "secondary" : "outline"}>{tab === "orders" ? String(row.financialStatus || row.paymentStatus || "-") : String(row.status || "draft")}</Badge></TableCell><TableCell>{tab === "returns" ? String(row.disposition || "Pending") : tab === "drafts" ? String(row.source || "Manual") : <Badge variant={orderQueue === "review" ? "destructive" : orderQueue === "ready-ship" ? "secondary" : "outline"}>{orderQueue.replace(/-/g, " ")}</Badge>}</TableCell><TableCell>{tab === "orders" ? <span className={`inline-flex items-center gap-1 text-xs font-medium ${hasPo ? "text-emerald-700" : "text-muted-foreground"}`} title={hasPo ? `Purchase order: ${Array.isArray(row.purchaseOrderNumbers) ? row.purchaseOrderNumbers.join(", ") : "linked"}` : "No purchase order linked"}>{hasPo ? <CheckCircle2 className="size-4" /> : <AlertCircle className="size-4" />}{hasPo ? "Linked" : "None"}</span> : <Badge variant="outline">{String((row.channelSync as Record<string, unknown> | undefined)?.status || "Local")}</Badge>}</TableCell><TableCell>{String(row.channelSource || row.salesChannel || row.source || "-")}</TableCell><TableCell>{dateLabel(String(row.updatedAt || row.createdAt || ""))}</TableCell><TableCell><DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" variant="ghost" title={`${tab === "orders" ? "Order" : tab === "drafts" ? "Quote" : "Return"} actions`}><MoreHorizontal className="size-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">{tab === "orders" && <><DropdownMenuItem onSelect={() => void openOrder(row)}>Quick view</DropdownMenuItem><DropdownMenuItem asChild><a href={`/orders/${encodeURIComponent(id || reference)}`}>Open order</a></DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => void runAction([id], "approve")}>Approve</DropdownMenuItem><DropdownMenuItem onSelect={() => void runAction([id], "hold")}>Put on hold</DropdownMenuItem><DropdownMenuItem onSelect={() => void createSupplierPos([id])}>Create supplier PO(s)</DropdownMenuItem><DropdownMenuItem onSelect={() => void createBackorder(row)}>Create backorder PO</DropdownMenuItem><DropdownMenuItem onSelect={() => void runAction([id], "done")}>Mark done</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => void runAction([id], "cancel")}>Cancel locally</DropdownMenuItem><DropdownMenuItem className="text-destructive focus:text-destructive" onSelect={() => void runAction([id], "void")}>Void locally</DropdownMenuItem></>}{tab === "drafts" && <><DropdownMenuItem onSelect={() => void duplicateDraft(row)}>Duplicate quote</DropdownMenuItem><DropdownMenuItem onSelect={() => void convertDraft(row)}>Convert to order</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem asChild><a href={`/api/order-drafts/${encodeURIComponent(id)}/export/pdf`} target="_blank" rel="noreferrer">Download quote</a></DropdownMenuItem></>}{tab === "returns" && <><DropdownMenuItem asChild><a href={`/orders/${encodeURIComponent(linkedOrderId)}`}>Open linked order</a></DropdownMenuItem>{String(row.source || "").toLowerCase() === "shopify" && <DropdownMenuItem onSelect={() => void api(`/api/returns/${encodeURIComponent(id)}/sync-shopify`, { method: "POST" }).then(() => { toast.success("Return sent to Shopify."); return load() }).catch((error: Error) => toast.error(error.message))}>Send to Shopify</DropdownMenuItem>}</>}</DropdownMenuContent></DropdownMenu></TableCell></TableRow> })}{!filtered.length && <TableRow><TableCell colSpan={12} className="h-28 text-center text-muted-foreground">No records match the current queue and filters.</TableCell></TableRow>}</TableBody></Table></div>}
        </>}
      </CardContent>
    </Card>
