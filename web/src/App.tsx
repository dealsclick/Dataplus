...
function StandaloneProductPage() {
  ...
  return <CompleteProductWorkspace product={product} sku={sku} onBack={returnToCatalog} onUpdated={setProduct} />
...
function CompleteProductWorkspace({ product, sku, onBack, onUpdated }: { product: ProductItem; sku: string; onBack: () => void; onUpdated: (product: ProductItem) => void }) {
  ...
  const openEditor = () => { setDraft({ marketplaceTitle: product.marketplaceTitle || product.title || "", brand: product.brand || "", mainCategory: product.mainCategory || product.category || "", condition: product.condition || "New", tags: (product.tags || []).join(", "), shortDescription: product.shortDescription || "", longDescription: product.longDescription || "", bulletPoints: (product.bulletPoints || []).join("\n"), seoKeywords: product.seoKeywords || "", wildcardSearch: product.wildcardSearch || "", websitePrice: String(price), qty: String(product.qty ?? product.stockQty ?? 0), reorderPoint: String(product.reorderPoint || 0), itemLength: String(product.itemLength || 0), itemWidth: String(product.itemWidth || 0), itemHeight: String(product.itemHeight || 0), itemWeight: String(product.itemWeight || 0), packageLength: String(product.packageLength || 0), packageWidth: String(product.packageWidth || 0), packageHeight: String(product.packageHeight || 0), packageWeight: String(product.packageWeight || 0), countryOfOrigin: product.countryOfOrigin || "", sdsUrl: product.sdsUrl || "", hazardous: Boolean(product.hazardous) }); setEditorOpen(true) }
  const save = async () => { setSaving(true); try { const numeric = new Set(["websitePrice", "qty", "reorderPoint", "itemLength", "itemWidth", "itemHeight", "itemWeight", "packageLength", "packageWidth", "packageHeight", "packageWeight"]); const payload = Object.fromEntries(Object.entries(draft).map(([key, value]) => [key, numeric.has(key) ? Number(value || 0) : value])); const result = await api<{ item: ProductItem }>(`/api/inventory/${encodeURIComponent(product.sku || sku)}`, { method: "PATCH", body: JSON.stringify(payload) }); onUpdated(result.item); setEditorOpen(false); toast.success("Product details saved.") } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to save product details.") } finally { setSaving(false) } }
  ...
  return <div className="product-workspace grid gap-5">
    <ProductPageSummary product={product} sku={sku} imageUrls={imageUrls} price={price} margin={margin} available={available} onBack={onBack} onEdit={openEditor} />
    ...
    <Tabs defaultValue="overview"><div className="overflow-x-auto rounded-md border bg-card p-1"><TabsList className="h-auto min-w-max justify-start bg-transparent p-0"><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="content">Content</TabsTrigger><TabsTrigger value="pricing">Pricing</TabsTrigger><TabsTrigger value="inventory">Inventory</TabsTrigger><TabsTrigger value="shipping">Shipping & compliance</TabsTrigger><TabsTrigger value="shopify">Shopify</TabsTrigger><TabsTrigger value="offers">Variants & offers</TabsTrigger><TabsTrigger value="source">Source & history</TabsTrigger></TabsList></div>
      <TabsContent value="overview" className="mt-4 grid gap-4"><div className="grid gap-4 xl:grid-cols-2">{section("Identity & category", "Primary catalog and supplier identity.", values([["SKU", product.sku || sku], ["Vendor SKU", product.vendorSku || ""], ["Brand", product.brand || ""], ["Manufacturer", product.manufacturer || ""], ["Main category", product.mainCategory || product.category || ""], ["Vendor category", product.vendorCategory || product.sourceCategory || ""], ["UPC / barcode", product.barcode || ""], ["UNSPSC", String(product.unspsc || "")]]))}{section("Readiness", "Core requirements before marketplace publication.", values([["Category", product.categoryVerified ? "Ready" : "Needs mapping"], ["Content", product.longDescription ? "Ready" : "Needs description"], ["Images", `${imageUrls.length} available`], ["Shipping", product.shippingMethod || "Needs measurements"], ["Shopify", product.shopifyPublished ? "Live" : product.shopifyId ? "Linked" : "Not linked"], ["Status", product.toBeDiscontinued ? "Blocked - discontinued" : "Eligible"]]))}</div>{section("Product content", "Customer-facing merchandising content.", <div className="grid gap-4"><div><p className="mb-1 text-xs font-bold uppercase tracking-wide text-muted-foreground">Short description</p><p className="whitespace-pre-wrap text-sm leading-6">{product.shortDescription || "No short description has been prepared."}</p></div><Separator /><div><p className="mb-1 text-xs font-bold uppercase tracking-wide text-muted-foreground">Long description</p><p className="whitespace-pre-wrap text-sm leading-6">{product.longDescription || "No long description has been prepared."}</p></div></div>)}<div className="grid gap-4 xl:grid-cols-2">{section("Pricing summary", "Current rule result for the sell unit.", values([["Supplier cost", moneyLabel(product.cost)], ["Sell-unit cost", moneyLabel(product.sellUnitCost)], ["Website price", moneyLabel(price)], ["Gross profit", moneyLabel(price - cost)], ["Margin", `${margin.toFixed(1)}%`], ["Pricing basis", product.isMultiUnit ? "UOM sell unit" : "Each"]]))}{section("Inventory summary", "Physical and sellable inventory behavior.", values([["On hand", numberLabel(product.qty ?? product.stockQty)], ["Reserved", numberLabel(product.reserved)], ["Available", numberLabel(available)], ["Reorder point", numberLabel(product.reorderPoint)], ["Replenishable", product.replenishable ? "Enabled" : "Off"], ["Replenishable qty", numberLabel(product.effectiveReplenishableQty)]]))}</div></TabsContent>
      ...
      <TabsContent value="pricing" className="mt-4 grid gap-4">{section("Base pricing & rule output", "Current product and Shopify price comparison.", values([["Cost", moneyLabel(product.cost)], ["FOB", moneyLabel(product.fobPrice)], ["Sell-unit cost", moneyLabel(product.sellUnitCost)], ["Website price", moneyLabel(price)], ["List / MSRP", moneyLabel(product.listPrice || product.msrp)], ["Shopify system price", moneyLabel(product.shopifySystemPrice)], ["Shopify live price", moneyLabel(product.shopifyLivePrice)], ["Live price difference", moneyLabel(product.shopifyPriceDelta ?? undefined)]]))}{section("Shopify purchase variants", "Actual UOM-based sell units; Essendant stays UOM-only.", <ProductVariantsTable rows={product.shopifyPurchaseVariants || []} />)}</TabsContent>
      <TabsContent value="inventory" className="mt-4 grid gap-4">{section("Warehouse stock", "Warehouse-level quantities and reorder thresholds.", <ProductWarehouseTable rows={product.warehouseStock || []} />)}{section("Recent product changes", "Latest recorded import and operational changes.", <ProductChangesTable rows={product.recentChanges || []} />)}</TabsContent>
      ...
      <TabsContent value="shopify" className="mt-4 grid gap-4">{section("Shopify connection", "Product linkage, live state, and SKU-level publishing actions.", <><div className="mb-4">{values([["Product ID", product.shopifyId || "Not linked"], ["Variant ID", product.shopifyVariantId || ""], ["Status", product.shopifyStatus || ""], ["Published", product.shopifyPublished ? "Yes" : "No"], ["Live price", moneyLabel(product.shopifyLivePrice)], ["Live inventory", numberLabel(product.shopifyLiveInventoryQuantity)], ["Inventory variant", product.shopifyLiveVariantSku || ""], ["Sync source", product.shopifySyncSource || ""]])}</div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => pushShopify("/api/shopify/product-create", false, "Product review")}>Review product</Button><Button size="sm" variant="outline" onClick={() => pushShopify("/api/shopify/variant-price-push", false, "Price review")}>Review price</Button><Button size="sm" disabled={product.toBeDiscontinued || Boolean(product.shopifyId)} onClick={() => pushShopify("/api/shopify/product-create", true, "Product create")}>{product.shopifyId ? "Already linked" : "Create product"}</Button><Button size="sm" disabled={product.toBeDiscontinued} onClick={() => pushShopify("/api/shopify/variant-price-push", true, "Price push")}>Push price</Button>{product.shopifyOnlineStoreUrl ? <Button size="sm" variant="outline" asChild><a href={product.shopifyOnlineStoreUrl} target="_blank" rel="noreferrer">Open Shopify</a></Button> : null}</div></>)}</TabsContent>
    ...
    </Tabs><ProductEditorDialog open={editorOpen} onOpenChange={setEditorOpen} draft={draft} setDraft={setDraft} saving={saving} onSave={save} />
  </div>
}

function ProductVariantsTable({ rows }: { rows: Array<Record<string, unknown>> }) { return rows.length ? <div className="overflow-hidden rounded-md border"><Table><TableHeader><TableRow><TableHead>SKU</TableHead><TableHead>Option</TableHead><TableHead>UOM</TableHead><TableHead>Cost</TableHead><TableHead>System price</TableHead><TableHead>Live</TableHead></TableRow></TableHeader><TableBody>{rows.map((row, index) => <TableRow key={`${String(row.sku || "variant")}-${index}`}><TableCell>{String(row.shopifyVariantSku || row.sku || "-")}</TableCell><TableCell>{[row.optionName, row.optionValue].filter(Boolean).join(": ") || "-"}</TableCell><TableCell>{String(row.uomDisplay || row.uomQty || "Each")}</TableCell><TableCell>{moneyLabel(row.unitCost as number)}</TableCell><TableCell>{moneyLabel(row.price as number)}</TableCell><TableCell>{row.shopifyPublished ? "Published" : String(row.shopifyStatus || "Not linked")}</TableCell></TableRow>)}</TableBody></Table></div> : <p className="rounded-md border border-dashed p-5 text-sm text-muted-foreground">No variants are configured.</p> }
function ProductPageSummary({ product, sku, imageUrls, price, margin, available, onBack, onEdit }: { product: ProductItem; sku: string; imageUrls: string[]; price: number; margin: number; available: number; onBack: () => void; onEdit: () => void }) { return <div className="grid gap-4 xl:grid-cols-[340px_minmax(0,1fr)]"><aside className="grid content-start gap-4"> <Card><CardHeader className="border-b pb-3"><CardTitle className="text-sm">Vendor information</CardTitle><CardDescription>Purchasing identity and source classification.</CardDescription></CardHeader><CardContent className="grid gap-3 pt-4"><Detail label="Supplier" value={product.supplier || product.vendor || "-"} /><Detail label="Vendor SKU" value={product.vendorSku || "-"} /><Detail label="Brand" value={product.brand || "-"} /><Detail label="Manufacturer" value={product.manufacturer || "-"} /><Detail label="UOM" value={product.uomDisplay || "Each"} /><Detail label="Vendor category" value={product.vendorCategory || product.sourceCategory || "-"} /></CardContent></Card><Card><CardHeader className="border-b pb-3"><CardTitle className="text-sm">Pricing & inventory</CardTitle><CardDescription>Current sell-unit rule result.</CardDescription></CardHeader><CardContent className="grid gap-3 pt-4"><Detail label="Supplier cost" value={moneyLabel(product.cost)} /><Detail label="Sell-unit cost" value={moneyLabel(product.sellUnitCost)} /><Detail label="Website price" value={moneyLabel(price)} /><Detail label="Margin" value={`${margin.toFixed(1)}%`} /><Separator /><Detail label="Available" value={numberLabel(available)} /><Detail label="Replenishable qty" value={numberLabel(product.effectiveReplenishableQty)} /></CardContent></Card></aside><section className="grid gap-4"><Card><CardContent className="grid gap-5 p-5 lg:grid-cols-[150px_minmax(0,1fr)]"><div className="grid aspect-square place-items-center overflow-hidden rounded-md border bg-muted/40">{product.defaultImage || imageUrls[0] ? <img src={product.defaultImage || imageUrls[0]} alt={product.title || sku} className="size-full object-contain p-3" /> : <Boxes className="size-9 text-muted-foreground" />}</div><div className="min-w-0"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Approved catalog product</p><h1 className="mt-1 text-2xl font-semibold tracking-tight">{product.marketplaceTitle || product.title || "Untitled product"}</h1><p className="mt-2 font-mono text-sm font-medium">{product.sku || sku}</p></div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={onBack}>Back</Button><Button size="sm" onClick={onEdit}><Pencil className="size-4" /> Edit</Button></div></div><p className="mt-4 text-sm text-muted-foreground">{product.mainCategory || product.category || "Uncategorized"}</p>{product.tags?.length ? <div className="mt-3 flex flex-wrap gap-1">{product.tags.map((tag) => <Badge key={tag} variant="outline">{tag}</Badge>)}</div> : null}</div></CardContent></Card><Card><CardHeader className="border-b pb-3"><CardTitle className="text-sm">Product content</CardTitle><CardDescription>Customer-facing product copy.</CardDescription></CardHeader><CardContent className="grid max-w-4xl gap-5 pt-4"><div><p className="mb-1 text-xs font-bold uppercase tracking-wide text-muted-foreground">Short description</p><p className="whitespace-pre-wrap text-sm leading-6">{product.shortDescription || "No short description has been prepared."}</p></div><Separator /><div><p className="mb-1 text-xs font-bold uppercase tracking-wide text-muted-foreground">Long description</p><p className="whitespace-pre-wrap text-sm leading-6">{product.longDescription || "No long description has been prepared."}</p></div></CardContent></Card></section></div> }
...
function ProductEditorDialog({ open, onOpenChange, draft, setDraft, saving, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; draft: Record<string, string | boolean>; setDraft: React.Dispatch<React.SetStateAction<Record<string, string | boolean>>>; saving: boolean; onSave: () => void }) { const set = (key: string, value: string | boolean) => setDraft((current) => ({ ...current, [key]: value })); const input = (label: string, key: string, type = "text") => <div className="grid gap-1.5"><Label>{label}</Label><Input type={type} value={String(draft[key] ?? "")} onChange={(event) => set(key, event.target.value)} /></div>; return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>Edit product</DialogTitle><DialogDescription>Save the product record in one focused step. Supplier pricing and marketplace rules remain controlled by their respective settings.</DialogDescription></DialogHeader><div className="grid gap-5"><div className="grid gap-3 sm:grid-cols-2">{input("Product title", "marketplaceTitle")}{input("Brand", "brand")}{input("Main category", "mainCategory")}{input("Condition", "condition")}{input("Tags (comma separated)", "tags")}{input("Website price", "websitePrice", "number")}{input("On-hand quantity", "qty", "number")}{input("Reorder point", "reorderPoint", "number")}</div><div className="grid gap-1.5"><Label>Short description</Label><Textarea rows={3} value={String(draft.shortDescription ?? "")} onChange={(event) => set("shortDescription", event.target.value)} /></div><div className="grid gap-1.5"><Label>Long description</Label><Textarea rows={7} value={String(draft.longDescription ?? "")} onChange={(event) => set("longDescription", event.target.value)} /></div><div className="grid gap-1.5"><Label>Bullet points (one per line)</Label><Textarea rows={5} value={String(draft.bulletPoints ?? "")} onChange={(event) => set("bulletPoints", event.target.value)} /></div><div className="grid gap-3 sm:grid-cols-2">{input("SEO keywords", "seoKeywords")}{input("Wildcard search", "wildcardSearch")}{input("Country of origin", "countryOfOrigin")}{input("SDS URL", "sdsUrl")}</div><div className="grid gap-3 sm:grid-cols-4">{["itemLength", "itemWidth", "itemHeight", "itemWeight", "packageLength", "packageWidth", "packageHeight", "packageWeight"].map((key) => input(key.replace(/([A-Z])/g, " $1"), key, "number"))}</div><ToggleRow label="Hazardous product" description="Marks this record for compliance review." checked={Boolean(draft.hazardous)} disabled={false} onCheckedChange={(checked) => set("hazardous", checked)} /></div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button onClick={onSave} disabled={saving}>{saving ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />} Save product</Button></DialogFooter></DialogContent></Dialog> }
