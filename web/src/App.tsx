...
function OrderDetailWorkspace() {
  ...
  return <div className="grid gap-5">
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(280px,0.9fr)]"><Tabs defaultValue="items"><TabsList className="flex flex-wrap"><TabsTrigger value="items">Items &amp; P&amp;L</TabsTrigger><TabsTrigger value="fulfillment">Fulfillment ({shipments.length})</TabsTrigger><TabsTrigger value="purchase-orders">POs ({Math.max(purchaseOrderIds.length, purchaseOrderNumbers.length)})</TabsTrigger><TabsTrigger value="finance">Finance</TabsTrigger><TabsTrigger value="customer">Customer</TabsTrigger><TabsTrigger value="returns">Returns ({relatedReturns.length})</TabsTrigger><TabsTrigger value="documents">Documents ({documents.length})</TabsTrigger><TabsTrigger value="channel">Channel</TabsTrigger><TabsTrigger value="activity">Activity ({events.length})</TabsTrigger></TabsList>
      <TabsContent value="items" className="grid gap-4 pt-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Detail label="Items revenue" value={moneyLabel(Number(pnl.itemRevenue || 0))} /><Detail label="Shipping collected" value={moneyLabel(Number(pnl.shippingCollected || 0))} /><Detail label="Estimated COGS" value={moneyLabel(Number(pnl.estimatedCogs || 0))} /><Detail label="Gross margin" value={`${Number(pnl.grossMarginPercent || 0).toFixed(1)}%`} /></div><Card><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Ordered SKU</TableHead><TableHead>Sell unit</TableHead><TableHead>Qty</TableHead><TableHead>Unit price</TableHead><TableHead>Unit cost</TableHead><TableHead>Profit</TableHead></TableRow></TableHeader><TableBody>{lines.map((line, index) => { const local = (line.localProduct || {}) as Record<string, unknown>; const variant = (line.variant || {}) as Record<string, unknown>; const sku = String(line.sku || "-"); return <TableRow key={`${sku}-${index}`}><TableCell><div className="flex items-center gap-1"><p className="font-mono font-medium">{local.sku ? <a className="hover:underline" href={`/products/${encodeURIComponent(String(local.sku))}`}>{sku}</a> : sku}</p>{local.sku ? null : <AlertCircle className="size-3.5 shrink-0 text-amber-700" aria-label="No local catalog match" />}</div><p className="max-w-64 truncate text-xs text-muted-foreground">{String(line.title || "")}</p></TableCell><TableCell>{variant.sku ? <><p>{String(variant.label || "Variant")}</p><p className="text-xs text-muted-foreground">Parent {String(variant.parentSku || local.sku || "-")}</p></> : <span>{Number(line.sellUnitQty || 1)} unit</span>}</TableCell><TableCell>{numberLabel(Number(line.qty || 0))}</TableCell><TableCell>{moneyLabel(Number(line.price || 0))}</TableCell><TableCell>{moneyLabel(Number(line.unitCost || 0))}</TableCell><TableCell><p className={Number(line.grossProfit || 0) < 0 ? "text-destructive" : "font-medium"}>{moneyLabel(Number(line.grossProfit || 0))}</p><p className="text-xs text-muted-foreground">{Number(line.grossMarginPercent || 0).toFixed(1)}% margin</p></TableCell></TableRow> })}</TableBody></Table></div></CardContent></Card></TabsContent>
