...
function OrderDetailWorkspace() {
  ...
  return <div className="grid gap-5">
    ...
    <Card><CardHeader className="flex-row items-center justify-between gap-4"><div><CardTitle className="text-sm">Fulfillment</CardTitle><CardDescription>{shipments.length ? `${shipments.length} shipment${shipments.length === 1 ? "" : "s"} recorded.` : "Select items to fulfill or create a shipping label."}</CardDescription></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => openFulfill(true)}>Mark as fulfilled</Button>{String(order.source || "").toLowerCase() === "shopify" && <Button size="sm" onClick={openShippingLabel}><Truck className="size-4" /> Create shipping label</Button>}</div></CardHeader></Card>
