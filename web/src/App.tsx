...
function PurchasingPage() {
  ...
  const [loading, setLoading] = useState(true); const [tab, setTab] = useState("requirements"); const [query, setQuery] = useState("")
  ...
  useEffect(() => { void load() }, [])
  const requirements = (data.requirements || []).filter((row) => JSON.stringify(row).toLowerCase().includes(query.toLowerCase())); const pos = (data.purchaseOrders || []).filter((row) => JSON.stringify(row).toLowerCase().includes(query.toLowerCase()))
  const openRequirements = requirements.filter((row) => !["received", "closed", "canceled"].includes(String(row.status))).length
  return <div className="grid gap-5"><PageHeader eyebrow="Buyer operations" title="Purchasing" description="Customer demand, supplier commitments, and receipt progress in one buyer workspace." action={<Button size="sm" variant="outline" onClick={() => void load()} disabled={loading}>{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />} Refresh</Button>} /><div className="grid gap-3 sm:grid-cols-3"><Detail label="Open requirements" value={numberLabel(openRequirements)} /><Detail label="Buyer review" value={numberLabel(requirements.filter((row) => ["new", "buyer_review", "awaiting_approval"].includes(String(row.status))).length)} /><Detail label="Open POs" value={numberLabel(pos.filter((row) => !["received", "closed", "canceled"].includes(String(row.status))).length)} /></div><Tabs value={tab} onValueChange={setTab}><TabsList><TabsTrigger value="requirements">Purchase requirements ({numberLabel(requirements.length)})</TabsTrigger><TabsTrigger value="pos">Purchase orders ({numberLabel(pos.length)})</TabsTrigger></TabsList><div className="relative mt-4 max-w-xl"><Search className="absolute left-2.5 top-2.5 size-4 text-muted-foreground" /><Input className="pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search PO, order, supplier, customer, or SKU" /></div><TabsContent value="requirements" className="mt-4"><Card><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Customer order</TableHead><TableHead>SKU / quantity</TableHead><TableHead>Fulfillment path</TableHead><TableHead>Supplier</TableHead><TableHead>PO</TableHead><TableHead>Ship by</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{requirements.map((row) => <TableRow key={String(row.id)}><TableCell><a className="font-medium hover:underline" href={`/orders/${encodeURIComponent(String(row.orderId))}`}>{String(row.orderNumber)}</a><p className="text-xs text-muted-foreground">{String(row.customer || "Customer")}</p></TableCell><TableCell>{String(row.sku || "-")}<p className="text-xs text-muted-foreground">{numberLabel(Number(row.qty || 0))} units</p></TableCell><TableCell><Badge variant="outline">{String(row.type || "purchase").replace(/_/g, " ")}</Badge></TableCell><TableCell>{String(row.vendorName || "Unassigned")}</TableCell><TableCell>{String(row.purchaseOrderNumber || "Create PO")}</TableCell><TableCell>{String(row.shipBy || "-")}</TableCell><TableCell><Badge variant={String(row.status).includes("exception") ? "destructive" : "secondary"}>{String(row.status || "new").replace(/_/g, " ")}</Badge></TableCell></TableRow>)}{!requirements.length && <TableRow><TableCell colSpan={7} className="h-28 text-center text-muted-foreground">No purchase requirements match this view.</TableCell></TableRow>}</TableBody></Table></div></CardContent></Card></TabsContent><TabsContent value="pos" className="mt-4"><Card><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>PO</TableHead><TableHead>Supplier</TableHead><TableHead>Destination</TableHead><TableHead>Orders</TableHead><TableHead>Units</TableHead><TableHead>Status</TableHead><TableHead>Updated</TableHead></TableRow></TableHeader><TableBody>{pos.map((po) => <TableRow key={String(po.id)}><TableCell className="font-medium">{String(po.poNumber || po.id)}</TableCell><TableCell>{String(po.supplier || "Unassigned")}</TableCell><TableCell>{String(po.warehouseName || "-")}</TableCell><TableCell>{numberLabel(Array.isArray(po.orderIds) ? po.orderIds.length : 0)}</TableCell><TableCell>{numberLabel(Number(po.totalUnits || 0))}</TableCell><TableCell><Badge variant="outline">{String(po.status || "draft").replace(/_/g, " ")}</Badge></TableCell><TableCell>{dateLabel(String(po.updatedAt || po.createdAt || ""))}</TableCell></TableRow>)}{!pos.length && <TableRow><TableCell colSpan={7} className="h-28 text-center text-muted-foreground">No purchase orders match this view.</TableCell></TableRow>}</TableBody></Table></div></CardContent></Card></TabsContent></Tabs></div>
}

function PurchaseOrderDetailPage() {
  const id = decodeURIComponent((window.location.pathname.split("/")[2] || "").trim()); const [data, setData] = useState<{ purchaseOrder?: Record<string, unknown>; linkedOrders?: Array<Record<string, unknown>> }>({}); const [loading, setLoading] = useState(true)
  ...
  useEffect(() => { void load() }, [id]); const po = data.purchaseOrder || {}; const lines = Array.isArray(po.items) ? po.items as Array<Record<string, unknown>> : Array.isArray(po.lines) ? po.lines as Array<Record<string, unknown>> : []
  if (loading) return <div className="grid gap-3"><Skeleton className="h-28" /><Skeleton className="h-64" /></div>
  return <div className="grid gap-5"><PageHeader eyebrow="Purchasing / Purchase order" title={String(po.poNumber || id)} description={`${String(po.supplier || "Unassigned supplier")} to ${String(po.warehouseName || "unassigned destination")}`} action={<div className="flex gap-2"><Button size="sm" variant="outline" asChild><a href="/purchasing">Back to purchasing</a></Button><Button size="sm" variant="outline" onClick={() => void load()}><RefreshCw className="size-4" /> Refresh</Button></div>} /><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Detail label="Status" value={String(po.status || "draft").replace(/_/g, " ")} /><Detail label="Supplier" value={String(po.supplier || "Unassigned")} /><Detail label="Destination" value={String(po.warehouseName || "-")} /><Detail label="Units" value={numberLabel(Number(po.totalUnits || lines.reduce((sum, line) => sum + Number(line.qty || 0), 0)))} /></div><Tabs defaultValue="lines"><TabsList><TabsTrigger value="lines">Lines ({lines.length})</TabsTrigger><TabsTrigger value="orders">Linked orders ({(data.linkedOrders || []).length})</TabsTrigger><TabsTrigger value="receipts">Receipts ({Array.isArray(po.receipts) ? po.receipts.length : 0})</TabsTrigger><TabsTrigger value="history">History</TabsTrigger></TabsList><TabsContent value="lines" className="mt-4"><Card><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>SKU</TableHead><TableHead>Item</TableHead><TableHead>Ordered</TableHead><TableHead>Received</TableHead><TableHead>Remaining</TableHead></TableRow></TableHeader><TableBody>{lines.map((line, index) => <TableRow key={`${String(line.sku)}-${index}`}><TableCell><a className="font-medium hover:underline" href={`/products/${encodeURIComponent(String(line.sku || ""))}`}>{String(line.sku || "-")}</a></TableCell><TableCell>{String(line.title || "-")}</TableCell><TableCell>{numberLabel(Number(line.qty || 0))}</TableCell><TableCell>{numberLabel(Number(line.receivedQty || 0))}</TableCell><TableCell>{numberLabel(Number(line.remainingQty ?? Math.max(0, Number(line.qty || 0) - Number(line.receivedQty || 0))))}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card></TabsContent><TabsContent value="orders" className="mt-4 grid gap-2">{(data.linkedOrders || []).map((order) => <Card key={String(order.id)}><CardContent className="flex items-center justify-between p-4 text-sm"><a className="font-medium hover:underline" href={`/orders/${encodeURIComponent(String(order.id))}`}>{String(order.orderNumber || order.id)}</a><span>{String(order.buyer || "Customer")}</span><Badge variant="outline">{String(order.operationalStatus || order.status || "-").replace(/_/g, " ")}</Badge></CardContent></Card>)}</TabsContent><TabsContent value="receipts" className="mt-4"><Card><CardContent className="p-4 text-sm text-muted-foreground">Receiving is recorded against this PO and automatically releases linked customer demand into fulfillment.</CardContent></Card></TabsContent><TabsContent value="history" className="mt-4"><Card><CardContent className="p-4 text-sm text-muted-foreground">{Array.isArray(po.timeline) && po.timeline.length ? String(po.timeline[0]?.message || po.timeline[0]?.title || "") : "No PO activity has been recorded."}</CardContent></Card></TabsContent></Tabs></div>
}
