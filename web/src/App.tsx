...
function WarehouseAuditHistory() {
  ...
  useEffect(() => { void load() }, [])
  const selectedLines = Array.isArray(selectedAudit?.lines) ? selectedAudit.lines as Array<Record<string, unknown>> : []
  const selectedUnknown = Array.isArray(selectedAudit?.unknownBarcodes) ? selectedAudit.unknownBarcodes as Array<Record<string, unknown>> : []
  return <><Card><CardHeader className="flex-row items-start justify-between gap-3"><div><CardTitle className="text-base">Audit register</CardTitle><CardDescription>Every warehouse count is retained here. In-progress audits automatically reopen in the scanner workspace below.</CardDescription></div><Button size="sm" variant="outline" disabled={loading} onClick={() => void load()}>{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />} Refresh</Button></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Audit</TableHead><TableHead>Warehouse</TableHead><TableHead>Status</TableHead><TableHead>Counted SKUs</TableHead><TableHead>Unknown UPCs</TableHead><TableHead>Started</TableHead><TableHead>Completed</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader><TableBody>{audits.map((audit) => { const lines = Array.isArray(audit.lines) ? audit.lines : []; const unknown = Array.isArray(audit.unknownBarcodes) ? audit.unknownBarcodes : []; const inProgress = String(audit.status) === "in_progress"; return <TableRow key={String(audit.id)}><TableCell><button className="font-medium hover:underline" onClick={() => setSelectedAudit(audit)}>{String(audit.auditNumber || "Audit")}</button></TableCell><TableCell>{String(audit.warehouseName || "-")}</TableCell><TableCell><Badge variant={inProgress ? "secondary" : "outline"}>{inProgress ? "In progress - resume below" : String(audit.status || "unknown").replace(/_/g, " ")}</Badge></TableCell><TableCell>{numberLabel(lines.length)}</TableCell><TableCell>{numberLabel(unknown.length)}</TableCell><TableCell>{dateLabel(String(audit.createdAt || ""))}</TableCell><TableCell>{audit.completedAt ? dateLabel(String(audit.completedAt)) : "-"}</TableCell><TableCell className="text-right"><Button size="sm" variant="outline" onClick={() => setSelectedAudit(audit)}>View</Button></TableCell></TableRow> })}{!audits.length && <TableRow><TableCell colSpan={8} className="h-20 text-center text-muted-foreground">No warehouse audits have been created yet.</TableCell></TableRow>}</TableBody></Table></div></CardContent></Card><Dialog open={Boolean(selectedAudit)} onOpenChange={(open) => !open && setSelectedAudit(null)}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-5xl"><DialogHeader><DialogTitle>{String(selectedAudit?.auditNumber || "Warehouse audit")}</DialogTitle><DialogDescription>{String(selectedAudit?.warehouseName || "Warehouse")} / {String(selectedAudit?.status || "unknown").replace(/_/g, " ")} / started {dateLabel(String(selectedAudit?.createdAt || ""))}</DialogDescription></DialogHeader><div className="grid gap-5"><div><p className="mb-2 text-sm font-semibold">Counted line items</p><div className="overflow-x-auto rounded-md border"><Table><TableHeader><TableRow><TableHead>SKU</TableHead><TableHead>Product</TableHead><TableHead>UPC scanned</TableHead><TableHead>Expected</TableHead><TableHead>Counted</TableHead><TableHead>Variance</TableHead><TableHead>Last scan</TableHead></TableRow></TableHeader><TableBody>{selectedLines.map((line) => <TableRow key={String(line.productId || line.sku)}><TableCell><a className="font-medium hover:underline" href={`/products/${encodeURIComponent(String(line.sku || ""))}`}>{String(line.sku || "-")}</a></TableCell><TableCell>{String(line.title || "-")}</TableCell><TableCell>{String(line.barcode || "-")}</TableCell><TableCell>{numberLabel(Number(line.expectedQty || 0))}</TableCell><TableCell>{numberLabel(Number(line.countedQty || 0))}</TableCell><TableCell><Badge variant={Number(line.countedQty || 0) === Number(line.expectedQty || 0) ? "outline" : "secondary"}>{numberLabel(Number(line.countedQty || 0) - Number(line.expectedQty || 0))}</Badge></TableCell><TableCell>{dateLabel(String(line.lastScannedAt || line.firstScannedAt || ""))}</TableCell></TableRow>)}{!selectedLines.length && <TableRow><TableCell colSpan={7} className="h-20 text-center text-muted-foreground">No catalog line items have been scanned yet.</TableCell></TableRow>}</TableBody></Table></div></div><div><p className="mb-2 text-sm font-semibold">Unresolved UPCs</p><div className="overflow-x-auto rounded-md border"><Table><TableHeader><TableRow><TableHead>UPC</TableHead><TableHead>Scans</TableHead><TableHead>Source catalog match</TableHead><TableHead>Last scanned</TableHead></TableRow></TableHeader><TableBody>{selectedUnknown.map((line, index) => { const sourceMatch = line.sourceMatch as Record<string, unknown> | null; return <TableRow key={`${String(line.barcode)}-${index}`}><TableCell className="font-medium">{String(line.barcode || "-")}</TableCell><TableCell>{numberLabel(Number(line.count || 0))}</TableCell><TableCell>{sourceMatch ? `${String(sourceMatch.sku || sourceMatch.vendorSku || "Source item")} / ${String(sourceMatch.title || "")}` : "No match"}</TableCell><TableCell>{dateLabel(String(line.lastScannedAt || line.scannedAt || ""))}</TableCell></TableRow> })}{!selectedUnknown.length && <TableRow><TableCell colSpan={4} className="h-16 text-center text-muted-foreground">No unresolved UPCs in this audit.</TableCell></TableRow>}</TableBody></Table></div></div></div><DialogFooter><Button onClick={() => setSelectedAudit(null)}>Close</Button></DialogFooter></DialogContent></Dialog></>
}
...
function WarehouseAuditPanel() {
  ...
  return <Card><CardHeader><CardTitle className="text-base">Warehouse audit</CardTitle><CardDescription>Start a count, scan UPCs with a phone camera or keyboard-wedge scanner, then apply verified inventory counts.</CardDescription></CardHeader><CardContent className="grid gap-4">{!current ? <div className="flex flex-wrap items-end gap-2"><Field label="Warehouse"><Input value={warehouse} onChange={(event) => setWarehouse(event.target.value)} /></Field><Button disabled={busy || !warehouse.trim()} onClick={() => void create()}>Start warehouse audit</Button></div> : <><div className="flex flex-wrap items-center justify-between gap-3 rounded-md border bg-muted/20 p-3"><div><p className="font-medium">{String(current.auditNumber)}</p><p className="text-xs text-muted-foreground">{String(current.warehouseName)} · {numberLabel(lines.length)} catalog SKUs counted · {numberLabel(unknowns.length)} unknown UPCs</p></div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => setCameraOpen((value) => !value)}>{cameraOpen ? "Stop camera" : "Use phone camera"}</Button><Button size="sm" disabled={busy || !lines.length} onClick={() => void complete()}>Complete & apply counts</Button></div></div>{cameraOpen && <video ref={videoRef} className="max-h-72 w-full rounded-md bg-black object-contain" muted playsInline />}<div className="flex gap-2"><Input autoFocus value={barcode} onChange={(event) => setBarcode(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void submit() } }} placeholder="Scan or enter UPC, then press Enter" /><Button disabled={busy || !barcode.trim()} onClick={() => void submit()}>Add scan</Button></div>{lastScan && <div className={`rounded-md border p-3 text-sm ${lastScan.matched ? "border-emerald-200 bg-emerald-50 text-emerald-950" : "border-amber-200 bg-amber-50 text-amber-950"}`}><span className="font-medium">{lastScan.matched ? "Catalog item found." : "Catalog item not found."}</span> {lastScan.message} <span className="ml-1 font-mono text-xs">{lastScan.barcode}</span></div>}{manualUnknown && <div className="grid gap-3 rounded-md border border-amber-300 bg-amber-50/50 p-3"><div><p className="font-medium">Add unmatched item details</p><p className="text-xs text-muted-foreground">The UPC is retained with this audit and can be reconciled to the catalog later.</p></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Field label="UPC"><Input value={manualUnknown.barcode} disabled /></Field><Field label="SKU"><Input value={manualUnknown.sku} onChange={(event) => setManualUnknown((entry) => entry ? { ...entry, sku: event.target.value } : entry)} placeholder="Optional SKU" /></Field><Field label="Product name"><Input value={manualUnknown.title} onChange={(event) => setManualUnknown((entry) => entry ? { ...entry, title: event.target.value } : entry)} placeholder="What was scanned?" /></Field><Field label="Location"><Input value={manualUnknown.locationBin} onChange={(event) => setManualUnknown((entry) => entry ? { ...entry, locationBin: event.target.value } : entry)} placeholder="Bin or area" /></Field><Field label="Quantity"><Input type="number" min="1" value={manualUnknown.qty} onChange={(event) => setManualUnknown((entry) => entry ? { ...entry, qty: event.target.value } : entry)} /></Field></div><div className="flex justify-end gap-2"><Button size="sm" variant="outline" onClick={() => setManualUnknown(null)}>Skip for now</Button><Button size="sm" disabled={busy || Number(manualUnknown.qty || 0) <= 0} onClick={() => void saveManualUnknown()}>Save unmatched item</Button></div></div>}<div className="overflow-x-auto rounded-md border"><Table><TableHeader><TableRow><TableHead>SKU</TableHead><TableHead>Product</TableHead><TableHead>Expected</TableHead><TableHead>Counted</TableHead><TableHead>Variance</TableHead></TableRow></TableHeader><TableBody>{lines.map((line) => <TableRow key={String(line.productId || line.sku)}><TableCell className="font-medium">{String(line.sku || "-")}</TableCell><TableCell>{String(line.title || "-")}</TableCell><TableCell>{numberLabel(Number(line.expectedQty || 0))}</TableCell><TableCell>{numberLabel(Number(line.countedQty || 0))}</TableCell><TableCell><Badge variant={Number(line.countedQty || 0) === Number(line.expectedQty || 0) ? "outline" : "secondary"}>{numberLabel(Number(line.countedQty || 0) - Number(line.expectedQty || 0))}</Badge></TableCell></TableRow>)}{!lines.length && <TableRow><TableCell colSpan={5} className="h-20 text-center text-muted-foreground">Scan the first UPC to begin counting.</TableCell></TableRow>}</TableBody></Table></div>{unknowns.length > 0 && <div className="overflow-x-auto rounded-md border"><Table><TableHeader><TableRow><TableHead>Unmatched UPC</TableHead><TableHead>SKU</TableHead><TableHead>Product</TableHead><TableHead>Location</TableHead><TableHead className="text-right">Quantity</TableHead></TableRow></TableHeader><TableBody>{unknowns.map((item) => <TableRow key={String(item.barcode)}><TableCell className="font-mono">{String(item.barcode || "-")}</TableCell><TableCell>{String(item.manualSku || "-")}</TableCell><TableCell>{String(item.manualTitle || "-")}</TableCell><TableCell>{String(item.locationBin || "-")}</TableCell><TableCell className="text-right">{numberLabel(Number(item.count || 0))}</TableCell></TableRow>)}</TableBody></Table></div>}</>}</CardContent></Card>
}
