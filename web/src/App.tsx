...
function ProductFilterSelect({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) {
  const optionLabel = (option: string) => option === "true" ? (label === "Status" ? "Active" : "In stock") : option === "false" ? (label === "Status" ? "Inactive" : "Out of stock") : option.replace(/^shopify-/, "").replace(/\b\w/g, (letter) => letter.toUpperCase()).replace(/-/g, " ")
  return <div className="grid gap-1"><Label className="text-[11px]">{label}</Label><Select value={value || "all"} onValueChange={(next) => onChange(next === "all" ? "" : next)}><SelectTrigger className="h-8 min-w-36 text-xs"><SelectValue placeholder={`All ${label.toLowerCase()}`} /></SelectTrigger><SelectContent><SelectItem value="all">All {label.toLowerCase()}</SelectItem>{options.filter(Boolean).map((option) => <SelectItem key={option} value={option}>{optionLabel(option)}</SelectItem>)}</SelectContent></Select></div>
}
...
function MainCatalogPage({ inventoryOnly = false }: { inventoryOnly?: boolean }) {
  ...
  const updateDraftFilter = (key: string, value: string) => setDraftFilters((current) => ({ ...current, [key]: value }))
  const applyFilters = () => { setFilters(draftFilters); load(1, draftFilters) }
  const resetFilters = () => { setDraftFilters({}); setFilters({}); setQuery(""); load(1, {}) }
  const shopifyOptions = ["shopify-live", "shopify-linked", "shopify-missing", "shopify-ready", "shopify-not-ready", "shopify-unpublished"]
  const inventoryOptions = [{ label: "In stock", value: "true" }, { label: "Out of stock", value: "false" }]
  return <div className="grid gap-5"><PageHeader eyebrow="Catalog" title={title} description={description} action={<Button variant="outline" onClick={() => load()} disabled={loading}>{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />} Refresh</Button>} /><Card><CardHeader className="grid gap-3 border-b"><div className="flex flex-wrap gap-2"><div className="relative"><Search className="absolute left-2 top-2.5 size-4 text-muted-foreground" /><Input className="w-[380px] max-w-[70vw] pl-8" placeholder="Search SKU, title, barcode, vendor SKU" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") load(1) }} /></div><Button onClick={() => load(1)} disabled={loading}>Search</Button></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6"><ProductFilterSelect label="Shopify" value={draftFilters.channelStatus || ""} options={shopifyOptions} onChange={(value) => updateDraftFilter("channelStatus", value)} /><ProductFilterSelect label="Inventory" value={draftFilters.hasStock || ""} options={inventoryOptions.map((item) => item.value)} onChange={(value) => updateDraftFilter("hasStock", value)} /><ProductFilterSelect label="Supplier" value={draftFilters.supplier || ""} options={facets.suppliers || []} onChange={(value) => updateDraftFilter("supplier", value)} /><ProductFilterSelect label="Brand" value={draftFilters.brand || ""} options={facets.brands || []} onChange={(value) => updateDraftFilter("brand", value)} /><ProductFilterSelect label="Manufacturer" value={draftFilters.manufacturer || ""} options={facets.manufacturers || []} onChange={(value) => updateDraftFilter("manufacturer", value)} /><ProductFilterSelect label="Status" value={draftFilters.active || ""} options={["true", "false"]} onChange={(value) => updateDraftFilter("active", value)} /></div><div className="flex items-center gap-2"><Button size="sm" onClick={applyFilters} disabled={loading}>Apply filters{filterCount ? ` (${filterCount})` : ""}</Button><Button size="sm" variant="ghost" onClick={resetFilters} disabled={loading}>Clear</Button></div></CardHeader><CardContent className="p-4">{loading ? <div className="grid gap-2"><Skeleton className="h-12" /><Skeleton className="h-12" /><Skeleton className="h-12" /></div> : <><div className="mb-3 text-xs text-muted-foreground">{numberLabel(total)} approved SKU{total === 1 ? "" : "s"} / page {page} of {totalPages}</div><div className="overflow-x-auto rounded-md border"><Table><TableHeader><TableRow><TableHead>SKU</TableHead><TableHead>Product</TableHead><TableHead>Supplier</TableHead><TableHead>Brand</TableHead><TableHead>Category</TableHead><TableHead>Stock</TableHead><TableHead>Price</TableHead><TableHead>Shopify</TableHead><TableHead /></TableRow></TableHeader><TableBody>{rows.map((item) => <TableRow key={item.sku}><TableCell className="font-medium">{item.sku}</TableCell><TableCell className="max-w-80"><p className="truncate">{item.title || "Untitled"}</p></TableCell><TableCell>{item.supplier || item.vendor || "-"}</TableCell><TableCell>{item.brand || "-"}</TableCell><TableCell className="max-w-64"><p className="truncate">{item.mainCategory || item.category || "Uncategorized"}</p></TableCell><TableCell>{numberLabel(item.qty ?? item.stockQty)}</TableCell><TableCell>{moneyLabel(item.websitePrice ?? item.price)}</TableCell><TableCell><Badge variant={item.shopifyId ? "default" : "outline"}>{item.shopifyId ? (item.shopifyPublished ? "Live" : "Linked") : "Not linked"}</Badge></TableCell><TableCell><Button size="icon" variant="ghost" onClick={() => setSelected(item)} title="Open SKU"><MoreHorizontal className="size-4" /></Button></TableCell></TableRow>)}{!rows.length && <TableRow><TableCell colSpan={9} className="py-10 text-center text-muted-foreground">No approved products match these filters.</TableCell></TableRow>}</TableBody></Table></div><div className="mt-3 flex items-center justify-end gap-2"><Button size="sm" variant="outline" disabled={loading || page <= 1} onClick={() => load(page - 1)}>Previous</Button><Button size="sm" variant="outline" disabled={loading || page >= totalPages} onClick={() => load(page + 1)}>Next</Button></div></>}</CardContent></Card><ProductDetailSheet sourceItem={selected} open={Boolean(selected)} onOpenChange={(open) => { if (!open) setSelected(null) }} /></div>
}
