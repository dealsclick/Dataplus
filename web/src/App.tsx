...
function AdvancedMainCatalogPage({ totalSkuCount = 0 }: { totalSkuCount?: number }) {
  ...
  const [visible, setVisible] = useState<Record<string, boolean>>(() => {
    const defaults = { readiness: true, stock: true, price: true, brand: true, category: true, shopify: true, ebay: true, images: true, updated: true, created: false, alternates: false, manufacturer: false, vendorSku: false, status: false, shadows: false, uom: false, shipping: true }
    try { return { ...defaults, ...JSON.parse(window.localStorage.getItem("dataplus-products-columns") || "{}") } } catch { return defaults }
  })
  const columns = [
    ["readiness", "Readiness"], ["stock", "Stock"], ["price", "Price"], ["brand", "Brand"], ["category", "Category"], ["shopify", "Shopify"], ["ebay", "eBay"], ["images", "Images"], ["updated", "Updated"], ["created", "Created"], ["alternates", "Alternates"], ["manufacturer", "Manufacturer"], ["vendorSku", "Vendor SKU"], ["status", "Status"], ["shadows", "Shadows"], ["uom", "UOM"], ["shipping", "Shipping"],
  ] as const
  const filterDefinitions: Record<string, { label: string; values: string[]; display: (value: string) => string }> = {
