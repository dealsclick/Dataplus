...
function ShippingLabelReadinessPanel({ orderId }: { orderId: string }) {
  const [loading, setLoading] = useState(false)
  ...
  return <Card><CardHeader><CardTitle className="text-sm">Shopify Shipping label readiness</CardTitle><CardDescription>Checks the order and channel prerequisites. It does not purchase a label.</CardDescription></CardHeader><CardContent className="grid gap-3"><Button size="sm" variant="outline" className="w-fit" onClick={() => void check()} disabled={loading}>{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />} Check readiness</Button>{result && <div className="rounded-md border p-3 text-sm"><Badge variant={result.ready ? "default" : "outline"}>{result.ready ? "Ready for label purchase" : "Needs setup"}</Badge><p className="mt-2 text-muted-foreground">Shopify API: {result.apiVersion || "Not configured"}</p>{!result.ready && <ul className="mt-2 list-disc space-y-1 pl-5 text-muted-foreground">{(result.blockers || []).map((blocker) => <li key={blocker}>{blocker}</li>)}</ul>}</div>}</CardContent></Card>
}
  ...
  return <div className="grid gap-5">
    ...
    <Tabs defaultValue="items"><TabsList className="flex flex-wrap"><TabsTrigger value="items">Items & P&amp;L</TabsTrigger><TabsTrigger value="fulfillment">Fulfillment ({shipments.length})</TabsTrigger><TabsTrigger value="finance">Finance</TabsTrigger><TabsTrigger value="customer">Customer</TabsTrigger><TabsTrigger value="returns">Returns ({relatedReturns.length})</TabsTrigger><TabsTrigger value="documents">Documents ({documents.length})</TabsTrigger><TabsTrigger value="channel">Channel</TabsTrigger><TabsTrigger value="activity">Activity ({events.length})</TabsTrigger></TabsList>
      ...
      <TabsContent value="fulfillment" className="grid gap-4 pt-4">
        ...
        <ShippingLabelReadinessPanel orderId={orderId} />
