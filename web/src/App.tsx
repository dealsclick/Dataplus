    ...
    <Card>
      <CardHeader className="grid gap-3 border-b">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select value={filters.creationSource || "all"} onValueChange={(value) => { const next = { ...filters, creationSource: value === "all" ? "" : value }; setFilters(next); setAllFiltered(false); setSelectedIds(new Set()); void load(1, next) }}><SelectTrigger className="h-8 w-44"><SelectValue placeholder="Created from" /></SelectTrigger><SelectContent><SelectItem value="all">Any creation source</SelectItem><SelectItem value="source catalog / data dump">Source data dump</SelectItem><SelectItem value="csv product import">CSV product import</SelectItem><SelectItem value="warehouse audit">Warehouse audit</SelectItem><SelectItem value="legacy catalog import">Legacy catalog import</SelectItem></SelectContent></Select>
