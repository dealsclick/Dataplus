...
<div><h3 className="font-medium">Choose what DataPlus can do</h3><p className="text-sm text-muted-foreground">Select the features you want, then save. You can use order import without launching products. Scheduled imports run automatically only when selected.</p></div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            {([['channelEnabled', 'Enable Walmart channel'], ['walmartOrdersEnabled', 'Allow order import'], ['walmartLaunchEnabled', 'Catalog launch (review required)'], ['walmartOrderScheduleEnabled', 'Schedule order imports'], ['walmartInventoryEnabled', 'Allow inventory updates and inactive-item protection'], ['walmartInventoryScheduleEnabled', 'Schedule linked-listing inventory sync'], ['walmartPriceEnabled', 'Allow reviewed price updates'], ['walmartOrderUpdatesEnabled', 'Allow order acknowledgment and tracking']] as const).map(([key, label]) => <label className="flex gap-2 items-center text-sm" key={key}><input type="checkbox" checked={key === 'walmartInventoryScheduleEnabled' ? rules[key] !== false : rules[key] === true} onChange={e => update(key, e.target.checked)} />{label}</label>)}
          ...
          <div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-2 text-sm">Order import interval (hours)<Input type="number" min="1" max="24" value={rules.walmartOrderScheduleHours || 1} onChange={e => update('walmartOrderScheduleHours', Number(e.target.value))} /></label><label className="grid gap-2 text-sm">Reconcile orders created in the last (days)<Input type="number" min="1" max="180" value={rules.walmartOrderLookbackDays || 30} onChange={e => update('walmartOrderLookbackDays', Number(e.target.value))} /></label><label className="grid gap-2 text-sm">Inventory sync interval (hours)<Input type="number" min="1" max="24" value={rules.walmartInventoryScheduleHours || 12} onChange={e => update('walmartInventoryScheduleHours', Number(e.target.value))} /><span className="text-xs text-muted-foreground">Runs independently of imports for active linked Walmart listings.</span></label></div>

          <p className="text-xs text-muted-foreground">Scheduled imports run in production only, rechecking the selected creation-date window. Use a longer manual range to refresh older orders. Last order job: {status.schedule?.jobId || 'None'} · Last inventory job: {status.inventorySchedule?.jobId || 'None'}.</p>

          <Button disabled={busy} onClick={() => void run(() => saveRules('shipping'))}>Save features and continue</Button>
        </TabsContent>
        <TabsContent value="shipping" className="space-y-4">
          <h3 className="font-medium">Download and map fulfillment centers</h3><p className="text-sm text-muted-foreground">Walmart calls fulfillment centers shipping nodes. Download your account's nodes, then select the one served by your DataPlus physical warehouse or supplier stock location. This step is needed for inventory updates.</p>
          ...
          <div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-2 text-sm">Inventory stock source<select className="h-9 rounded border bg-background px-3" value={rules.walmartWarehouseId || ''} onChange={e => update('walmartWarehouseId', e.target.value)}><option value="">Select a warehouse</option>{warehouses.filter(w => (w.isPhysical === true || w.inventorySourceType === 'supplier_feed') && w.active !== false && w.status !== 'inactive').map(w => <option key={w.id} value={w.id}>{w.name}</option>)}</select></label><label className="grid gap-2 text-sm">Walmart fulfillment center<select className="h-9 min-w-0 w-full rounded border bg-background px-3" value={rules.walmartShipNode || ''} onChange={e => update('walmartShipNode', e.target.value)}><option value="">Select a downloaded shipping node</option>{rules.walmartShipNode && !(status.shipNodes?.rows || []).some((node: Json) => node.id === rules.walmartShipNode) && <option value={rules.walmartShipNode}>Saved node {rules.walmartShipNode} — download to verify</option>}{(status.shipNodes?.rows || []).map((node: Json) => <option key={node.id} value={node.id} disabled={node.status !== 'ACTIVE'}>{node.name} · {node.id} · {node.status}</option>)}</select></label><label className="grid gap-2 text-sm">Inventory safety quantity (sell units)<Input type="number" min="0" value={rules.walmartSafetyQty || 0} onChange={e => update('walmartSafetyQty', Number(e.target.value))} /></label><label className="grid gap-2 text-sm">Maximum published quantity (0 = uncapped)<Input type="number" min="0" value={rules.walmartMaxQuantity || 0} onChange={e => update('walmartMaxQuantity', Number(e.target.value))} /></label></div>

          <p className="text-xs text-muted-foreground">Inventory updates publish the selected source's unreserved stock divided by pack size. Supplier-feed stock uses this mapped supplier node and is held when the newest datadump is incomplete. Inactive products and blocked shipping classifications publish zero.</p>
        ...
        <TabsContent value="review" className="space-y-4"><h3 className="font-medium">Review your setup</h3><p className="text-sm">Connection: {status.connection?.verified ? 'Verified' : 'Needs verification'}</p><div className="grid gap-2 sm:grid-cols-2">{[['walmartLinkExistingEnabled', 'Automatic listing links'], ['walmartOrdersEnabled', 'Order import'], ['walmartOrderScheduleEnabled', 'Scheduled imports'], ['walmartLaunchEnabled', 'Catalog launch'], ['walmartInventoryEnabled', 'Inventory updates'], ['walmartInventoryScheduleEnabled', 'Scheduled inventory sync'], ['walmartPriceEnabled', 'Price updates'], ['walmartOrderUpdatesEnabled', 'Acknowledgment and tracking']].map(([key, label]) => <p className="flex justify-between gap-2 rounded border p-2 text-sm" key={key}>{label}<Badge variant={(key === 'walmartInventoryScheduleEnabled' ? rules[key] !== false : rules[key]) ? 'secondary' : 'outline'}>{(key === 'walmartInventoryScheduleEnabled' ? rules[key] !== false : rules[key]) ? 'Enabled' : 'Off'}</Badge></p>)}</div><p className="text-sm">Shipping node: {rules.walmartShipNode || 'Not mapped'} · Categories cached: {status.taxonomy?.count || 0}</p>{rules.walmartInventoryEnabled && (!rules.walmartShipNode || !rules.walmartWarehouseId) && <p className="text-sm text-amber-700 dark:text-amber-400">Map a stock source and an active Walmart node before inventory updates.</p>}<p className="text-sm text-muted-foreground">To launch products, select SKUs in Catalog and choose Actions → Launch on Walmart by UPC. Setup does not launch items.</p><Button disabled={busy || !dirty} onClick={() => void run(() => saveRules())}>Save setup</Button><Button asChild variant="outline"><a href="/products">Open Catalog</a></Button></TabsContent>
