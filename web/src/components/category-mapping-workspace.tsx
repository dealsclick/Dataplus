...
export function CategoryMappingWorkspace(props: Props) {
  ...
  return <div className="min-w-0 text-sm" aria-busy={busy || searching}>
    ...
    <section className="min-w-0 space-y-3 py-4">
      {props.onTreeSelect ? <ChannelCategoryPicker key={`${channel}:${props.localCategory}`} channel={channel} localCategory={props.localCategory} selectedId={props.categoryId} savedId={props.savedId} disabled={busy} readOnly={locked} onUnlock={props.onUnlock} onSelect={props.onTreeSelect} /> : <>
