...
type Batch = { rows: MatchRow[]; total: number; complete: boolean; status: string; message?: string; jobNumber?: number }
...
export function WalmartUpcMatch({ skus, open, onOpenChange }: { skus: string[]; open: boolean; onOpenChange: (open: boolean) => void }) {
  const [jobId, setJobId] = useState('')
  const [batch, setBatch] = useState<Batch | null>(null)
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')
  const selection = skus.join('\n')
  useEffect(() => { setJobId(''); setBatch(null); setError('') }, [selection])
  useEffect(() => {
    ...
    async function poll() {
      try {
        const result = await request(`match?jobId=${encodeURIComponent(jobId)}`, undefined, controller.signal)
  ...
  async function start() {
    ...
    try { const result = await request('match', { skus }); setJobId(result.job.id); setBatch({ rows: [], total: skus.length, complete: false, status: result.job.status, jobNumber: result.job.jobNumber, message: result.message }) }
  ...
  return <Dialog open={open} onOpenChange={value => { if (!busy) onOpenChange(value) }}><DialogContent className="flex max-h-[90dvh] w-[calc(100%_-_1rem)] flex-col overflow-hidden sm:max-w-3xl">
    <DialogHeader><DialogTitle>Match on Walmart by UPC</DialogTitle><DialogDescription>Search Walmart US for {skus.length} selected catalog item{skus.length === 1 ? '' : 's'} using saved UPC, EAN or GTIN. Matching does not create a listing or link a seller SKU.</DialogDescription></DialogHeader>
    <div className="min-h-0 space-y-3 overflow-y-auto">
      {error && <p role="alert" className="rounded border border-destructive p-3 text-sm text-destructive">{error}</p>}
      {!jobId && <p className="break-words rounded-md border bg-muted/30 p-3 text-sm">{skus.join(', ')}</p>}
      {batch && <div role="status" className="flex flex-wrap items-center gap-2 text-sm"><Badge variant="outline">{batch.status}</Badge><span>{batch.rows.length} / {batch.total} checked · {matched.length} matched</span><a className="underline" href="/jobs">Job {batch.jobNumber || jobId}</a>{batch.message && <p className="w-full text-muted-foreground">{batch.message}</p>}{batch.status === 'queued' && <p className="w-full text-muted-foreground">Waiting for the external worker. Results appear here as items are checked.</p>}</div>}
    ...
    <DialogFooter className="flex-wrap gap-2 border-t pt-3"><Button variant="outline" onClick={() => onOpenChange(false)} disabled={busy}>Close</Button>{matched.length > 0 && batch?.complete && <Button variant="outline" asChild><a href={`/products?action=walmart-launch&skus=${encodeURIComponent(matched.join('\n'))}`}>Review {matched.length} matched item{matched.length === 1 ? '' : 's'}</a></Button>}<Button disabled={busy || Boolean(active) || skus.length < 1 || skus.length > 100} onClick={() => void start()}>{busy ? <Loader2 className="size-4 animate-spin" /> : <Search className="size-4" />}{jobId ? 'Match again' : 'Match on Walmart by UPC'}</Button></DialogFooter>
