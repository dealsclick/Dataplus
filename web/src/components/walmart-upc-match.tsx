...
export function WalmartCatalogMatch({ sku, refreshKey }: { sku: string; refreshKey: boolean }) {
  const [row, setRow] = useState<Partial<MatchRow>>({}), [error, setError] = useState('')
  useEffect(() => {
    if (refreshKey) return
    const controller = new AbortController(); setRow({}); setError('')
  ...
  return <div className="mt-3 min-w-0 rounded border p-3"><p className="text-sm font-medium">Walmart catalog match</p><p className="text-xs text-muted-foreground">Catalog reference only. This does not mean your seller offer is linked or live.</p>{error ? <p className="text-sm text-destructive">{error}</p> : <><WalmartCatalogReference row={row} />{row.checkedAt && <p className="mt-1 text-xs text-muted-foreground">Checked {new Date(row.checkedAt).toLocaleString()}</p>}{row.status === 'unchecked' && <p className="text-xs">Run Match on Walmart by UPC to save the catalog reference.</p>}{row.status === 'not_found' && <p className="text-xs">No catalog match returned at the last check.</p>}</>}</div>
}
