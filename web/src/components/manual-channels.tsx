...
type Channel = { id: string; name: string; source_key: string; enabled: boolean }
...
export function ManualChannels({base,companyName,owner,picker=false,value='',onChange}:{base:string;companyName:string;owner?:boolean;picker?:boolean;value?:string;onChange?:(id:string)=>void}) {
  ...
  async function load(){const result=await request<{rows:Channel[]}>(`${base}/manual-channels`);setRows(result.rows)}
  useEffect(()=>{let cancelled=false;setLoading(true);request<{rows:Channel[]}>(`${base}/manual-channels`).then(data=>{if(!cancelled)setRows(data.rows)}).catch(e=>{if(!cancelled)setError(e.message)}).finally(()=>{if(!cancelled)setLoading(false)});return()=>{cancelled=true}},[base])
