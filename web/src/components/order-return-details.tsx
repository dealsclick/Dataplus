...
export function OrderReturnDetails({ record, trackingUrl, defaultTab = "details" }: { record: Row; trackingUrl: (row: Row, carrier: string, number: string) => string; defaultTab?: "details" | "transactions" }) {
  return <section className="min-w-0 basis-full border-t pt-3 text-sm">
    ...
    {rows(record.receiptHistory).length > 0 && <details className="mt-3 border-t pt-3"><summary className="cursor-pointer font-medium">Receiving history ({rows(record.receiptHistory).length})</summary>{rows(record.receiptHistory).map(receipt => <div key={String(receipt.id)} className="border-b py-2"><p>{String(receipt.at)} / {String(receipt.user)}</p><p>{String(receipt.condition)} / {String(receipt.disposition)} / {String(receipt.binLocation || "No bin")}</p>{rows(receipt.items).map((line, index) => <p key={index}>{String(line.sku)}: {Number(line.receivedQty || 0)} received total</p>)}</div>)}</details>}
    {rows(record.attachments).some(file => /^data:image\/(jpeg|png|webp);base64,/i.test(String(file.dataUrl || ""))) && <div className="mt-3 flex flex-wrap gap-2">{rows(record.attachments).filter(file => /^data:image\/(jpeg|png|webp);base64,/i.test(String(file.dataUrl || ""))).map(file => <a key={String(file.id)} href={String(file.dataUrl)} target="_blank" rel="noopener noreferrer"><img className="size-20 rounded object-cover" src={String(file.dataUrl)} alt={String(file.name || "Return photo")} /></a>)}</div>}
...
export function ReturnReceiptFields({ record, warehouses, draft, setDraft }: { record: Row | null; warehouses: Row[]; draft: Record<string, string>; setDraft: (next: Record<string, string>) => void }) {
  const physical = warehouses.filter((row) => row.isPhysical !== false && row.allowReceiving !== false && row.active !== false && String(row.status).toLowerCase() !== "inactive" && !/virtual|supplier|transfer/i.test(String(row.type || row.warehouseType || "") + String(row.inventorySourceType || "")) && !/datawarehouse/i.test(String(row.name || "")))
