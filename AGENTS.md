...
Supplier profiles may define multiple weekly cutoff-to-delivery windows in their configured timezone. Each purchase requirement, route, and Draft PO stores the selected cutoff date/time and expected delivery date so the buyer can see whether the cutoff is upcoming or has passed. Vendors without a weekly calendar use the daily cutoff fallback. True Value's default calendar is Tuesday 4:00 PM for Wednesday delivery and Friday 4:00 PM for Monday delivery; orders received after Friday cutoff or over the weekend roll to Tuesday cutoff and Wednesday delivery.

The selected supplier cutoff marks an open Draft PO as ready for buyer review or automatic submission when the supplier rules explicitly allow it. Cutoff does not delay draft creation. Buyers control submission unless the supplier profile has an explicit auto-submit rule. Purchasing groups Draft POs by canonical supplier, and `/purchasing/waiting-for-po/:supplierKey` shows each numbered draft with its linked customer orders, SKUs, quantities, images, destination, cutoff state, expected delivery, cost, and review state.
