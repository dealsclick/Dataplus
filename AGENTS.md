...
- Channel pricing methods preserve legacy rules by default. Explicit markup or gross-margin targets use sell-unit cost, not net-profit assumptions; preserve floors, freight allowances and SKU overrides. Configuration-only channels must be identified. See `docs/channel-pricing-methods.md`; changing settings must not queue repricing jobs automatically.

- Product overview selling-unit label, displayed cost, profit, and margin must use the primary pricing calculation (`sellUnit`, `primarySellUnitCost`, `finalPrice`) together, not mix individual prices with source-case costs. Preserve zero values. Run `node scripts/test-product-overview-pricing.cjs` when changing this display.
