...
- Supplier selling-unit permissions are configured in Vendor profile > Rules as independent checkboxes for Individual Each and Supplier-defined unit. The supplier-defined quantity is the largest valid UOM quantity, minimum quantity, or quantity increment; all values equal to one resolve to Each, while a value greater than one resolves to one complete pack or case. At least one permission is required. Checkbox combinations continue to persist through `variationRules.sellingUnitMode` for compatibility. Explicit choices govern product-page options and Shopify/eBay launch units without changing source cost basis, UOM, or stock allocation. Explicit Individual Each permission allows breaking a purchasing multiple; supplier-defined-only mode retains the restriction. Recorded purchasing multiples still define pack quantities (four four-packs consume sixteen units), and monetary minimum allowed prices remain enforced. Walmart is individual-only: block suppliers that do not permit Each, require individual UPC/GTIN confirmation for case-based sources, and never silently reprice an existing unverified case listing as an individual unit. See `docs/vendor-selling-units.md`; run the supplier-unit, eBay purchase-unit, Walmart selling-unit and pricing regression tests when changing this path.
...
eBay launch defaults to Match eBay catalog. Before creating an Inventory API item, search the Commerce Catalog using an exact GTIN or an exact brand-and-MPN pair and use an ePID only when one unambiguous exact product is returned. Cache identical lookups within the job and persist the match status, method and ePID with the listing record and result artifact. Never auto-select title/keyword-only or ambiguous results. A disabled match option always creates from DataPlus product content; no match or a lookup error falls back to that same from-scratch path without blocking launch.

If a SKU exists on eBay, show a View on eBay action when a listing URL is available.

Live eBay launches require a recent complete offers and live-status snapshot. When the latest complete GetMyeBaySelling active-listing feed is older than the configured launch freshness window, DataPlus queues that sync as a prerequisite, holds the launch, and continues automatically only after the prerequisite finishes successfully. Review/readiness jobs do not trigger this prerequisite. A failed, unavailable, incomplete, or capped feed must fail the dependent launch without publishing any SKU.

Pre-launch eBay status prerequisites are explicit background-lane jobs. They must not wait behind unrelated manual marketplace launches when the background worker is available.
...
Name readiness-only jobs **eBay launch preflight** so operators can find them consistently. Automatic post-import eBay launch preflights belong to the background worker lane and must not wait behind manual Shopify or marketplace publication jobs.
...
Non-order Walmart tasks use the dedicated `walmart` worker lane so large Shopify jobs cannot block Walmart feeds, pricing, reconciliation, taxonomy, or launch work. `walmart-orders` remains in the shared order lane. Pricing Insights follows Walmart's zero-based pagination, sends a stable sort, stays within the documented two-request-per-minute allowance, honors throttling, and converts persistent repeated pages into a partial warning that preserves prior cache data instead of claiming complete success.

`walmart-feed` jobs only poll the status of an existing submitted feed ID. Label them **Walmart feed status check** in job creation and display, and state clearly that they do not submit products or create another feed.
...
Temu order intake, status reconciliation and enrichment are separate jobs with independent configurable schedules and checkpoints. Intake only creates missing paid orders after an exact PostgreSQL identity check; status/enrichment never create orders or replace commerce/SKU mappings. New schedules default off. Preserve changed-only writes, bounded resume cursors, cancellation/channel guards and the shared single-active-job guard. Do not restore the old final full-order rewrite. Legacy Temu purchase orders may be split across multiple one-line local orders; status/enrichment must leave an ambiguous group unchanged, emit a per-order needs-review row, and continue the job rather than failing the entire reconciliation or silently selecting one child. See `docs/temu-order-jobs.md` and run `scripts/test-temu-order-phases.cjs` plus the Temu pagination/return-linking and source-completion tests for changes.
...
Walmart readiness separates local screening from remote catalog evidence. Local inactive/discontinued, supplier, selling-unit, shipping, identifier and price blockers must be recorded without calling Walmart. Background readiness reuses unchanged per-product assessments and a 24-hour identifier lookup cache, deduplicates concurrent UPC/GTIN lookups, and uses bounded concurrency. All-filtered readiness runs may locally screen every selected SKU but make at most 5,000 new Walmart catalog lookups per run; remaining locally eligible rows are saved as `remote_pending` and require a narrower follow-up selection. Stock is not a Walmart item-creation requirement. Progress must show locally screened rows, actual remote lookups, cache hits, local blockers and pending remote checks; never present an unbounded sequential API scan as normal progress.
