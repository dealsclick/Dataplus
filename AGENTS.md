...
eBay launch readiness must resolve duplicate local category documents deterministically without modifying protected mappings: an approved or protected mapping with a channel category ID takes precedence over an unmapped duplicate. Supplier minimum order quantities and quantity increments do not change what a source UPC identifies. For `EA`/Each products, the source UPC belongs to the Each selling unit; generated packs must not reuse that UPC and may use brand plus MPN when eBay permits it. Normalize and checksum GTIN values before sending them. Inventory readiness should report one sellable-quantity calculation rather than separate zero quantity, minimum inventory, and child-allocation messages for the same shortage.
