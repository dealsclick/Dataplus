...
Shipping classification rules live in System Settings > Catalog > Shipping classification. Dimensional weight is cost information only and never causes a selling block. Configurable defaults: long parcel above 48 in longest side or 105 in length plus girth; freight/outside parcel limits above 108 in, 165 in length plus girth, or 150 lb actual weight. Exact limits are inclusive. Honor explicit supplier freight fields, never derived shippingClass/shippingMethod as source evidence. Partial measurements remain reviewable; do not combine partial package and item dimensions. Product Edit > Shipping supports explicit Parcel/Long parcel/Freight overrides with a required reason and timestamp. Existing channel launch and inventory restriction switches remain independent and use current rules; Shopify draft creation also respects launch shipping restrictions. Saving rules changes local classification and future sync decisions, not live marketplace listings by itself. Do not automatically publish newly eligible products. Run scripts/test-shipping-classification.js when changing these rules.
